...
class TestStackInfo:
    ...
    def test_get_resources(self, mock_boto_client):
        ...
        mock_cfn.describe_stacks.return_value = {
            'Stacks': [{
                ...
                'Outputs': [
                    {'OutputKey': 'S3InputBucketName', 'OutputValue': 'input-bucket'},
                    {'OutputKey': 'S3OutputBucketName', 'OutputValue': 'output-bucket'},
                    {'OutputKey': 'S3ConfigurationBucketName', 'OutputValue': 'config-bucket'},
                    {'OutputKey': 'S3EvaluationBaselineBucketName', 'OutputValue': 'baseline-bucket'},
                    {'OutputKey': 'LambdaLookupFunctionName', 'OutputValue': 'lookup-function'},
                    {'OutputKey': 'StateMachineArn', 'OutputValue': 'arn:aws:states:us-east-1:123456789012:stateMachine:test'},
                ]
            }]
        }
    ...
    def test_get_resources_caching(self, mock_boto_client):
        ...
        with patch('boto3.client') as mock_client:
            mock_client.return_value.get_caller_identity.return_value = {'Account': '123456789012'}
