...
def show_final_summary(status_data: Dict, stats: Dict, elapsed_time: float):
    ...
    summary_table.add_row("Completed Successfully", str(stats['completed']), style="green")
...
def show_batch_submission_summary(results: Dict):
    ...
    if results['uploaded'] > 0:
        console.print(f"✓ Uploaded {results['uploaded']} documents to InputBucket", style="green")
    
    if results['queued'] > 0:
        console.print(f"✓ Sent {results['queued']} messages to processing queue", style="green")
...
def show_monitoring_instructions(stack_name: str, batch_id: str):
    ...
    console.print(f"  [cyan]idp-cli status --stack-name {stack_name} --batch-id {batch_id}[/cyan]")
    console.print()
    console.print("Or to wait for completion:")
    console.print(f"  [cyan]idp-cli status --stack-name {stack_name} --batch-id {batch_id} --wait[/cyan]")
    console.print()
...
def show_monitoring_header(batch_id: str):
    ...
    console.print()
    console.print("[dim]Press Ctrl+C to stop monitoring (processing will continue)[/dim]")
    console.print()


def create_live_display(batch_id: str, status_data: Dict, stats: Dict, elapsed_time: float) -> Table:
    """
