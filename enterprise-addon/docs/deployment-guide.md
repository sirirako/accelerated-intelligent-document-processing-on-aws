...
**Q4 — Enterprise Integration (Ping API Auth):**
"Do you need external systems to call the Jobs API using PingFederate tokens
(instead of Cognito)?  This is required for VPC-only deployments where Cognito's
OAuth domain is unavailable over PrivateLink.
...
If **yes**, collect (one at a time):
- `MQ_HOST`: "What is your RabbitMQ broker hostname? (e.g. b-xxxx.mq.us-east-1.amazonaws.com)"
- `MQ_EXCHANGE`: "What exchange should messages be published to? (empty = default exchange)"
- `MQ_ROUTING_KEY`: "What routing key? (default: idp.document.completed)"
- `MQ_OAUTH_SCOPE`: "What OAuth2 scope should be requested for the MQ connection? (e.g. idp-mq.write:%2F/idp-events)"
- `PING_TOKEN_URL`: "What is the Ping token endpoint for client-credentials? (e.g. https://sso.corp.example.com/as/token)"
- `MQ_PING_CLIENT_ID`: "What Ping client ID should the hook use?"
- `MQ_PING_CLIENT_SECRET_ARN`: "What is the Secrets Manager ARN containing the Ping client secret?"

Save these and add to the deploy parameters:
`EnableCompletionHook=true,CompletionHookMQHost=<MQ_HOST>,CompletionHookMQExchange=<MQ_EXCHANGE>,CompletionHookMQRoutingKey=<MQ_ROUTING_KEY>,CompletionHookMQOAuthScope=<MQ_OAUTH_SCOPE>,CompletionHookPingTokenUrl=<PING_TOKEN_URL>,CompletionHookPingClientId=<MQ_PING_CLIENT_ID>,CompletionHookPingClientSecretArn=<MQ_PING_CLIENT_SECRET_ARN>`
...
- If the user answered **from-source**, remind them to run `./enterprise-addon/build.sh`
  before publish (builds the enterprise Lambda layers).
...
This installs PyJWT (for the Ping JWT authorizer) and pika (for the RabbitMQ
completion hook) into the layer directories. SAM packages them automatically
during publish.
...
|---|---|---|
| PingFederate | Issuer URL, JWKS endpoint, audience/client_id | Identity team |
| Ping OAuth2 client (API) | Client configured with `client_credentials` grant + `jobs.read`/`jobs.write` scopes | Identity team |
| Amazon MQ broker (if using hook) | RabbitMQ hostname, OAuth2 backend configured with Ping as issuer | Messaging/infra team |
| Ping OAuth2 client (MQ hook) | Client with RabbitMQ-scoped claims; secret in Secrets Manager | Identity team |
| VPC networking | Lambda SG allows outbound to Ping (HTTPS) and MQ broker (port 5671) | Network team |
...
| **Ruff linting fails on enterprise-addon/layers** | Third-party packages in layer dirs picked up by linter | Ensure `enterprise-addon/layers/*/python` is in `extend-exclude` in `ruff.toml` |
| **401 Unauthorized on Jobs API (Ping auth enabled)** | Lambda SG blocks outbound HTTPS to Ping JWKS | Add outbound HTTPS rule to Lambda SG for Ping endpoint |
| **Completion hook timeout** | Lambda SG blocks outbound to MQ broker port 5671 | Add outbound rule to Lambda SG for MQ broker |
...
### Enterprise Integration — Verify

After deploying with enterprise features enabled, verify they work:
...
1. Submit a document (via API or Web UI)
2. Wait for processing to complete
3. Check your RabbitMQ consumer received a message with `document_id`, `status`, `results_location`

---

### Enterprise Integration — Troubleshooting
...
| Enterprise layer build fails | Missing pip or wrong platform | Re-run `./enterprise-addon/build.sh` with Python 3.12 |
...
| Enterprise API reference | `enterprise-addon/API.md` |
| Enterprise overview | `enterprise-addon/README.md` |
| Upstream sync guide | `enterprise-addon/docs/upstream-sync-guide.md` |
