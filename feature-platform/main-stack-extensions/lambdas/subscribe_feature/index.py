...
Env vars:
    SIMULATOR_ADMIN_ENDPOINT     Base URL of the simulator (e.g. https://sim.example.com).
                                 ...
                                 FEATURE_MARKETPLACE_URL_MAP is required.
    FEATURE_PRODUCT_CODE_MAP     JSON map {featureId: productCode}.
    FEATURE_OFFER_ID_MAP         JSON map {featureId: offerId}. Optional; simulator
                                 auto-creates a default public offer if missing.
    FEATURE_MARKETPLACE_URL_MAP  JSON map {featureId: realMarketplaceUrl}.
                                 Required in "marketplace" mode.
...
_FEATURE_MARKETPLACE_URL_MAP_RAW = os.environ.get("FEATURE_MARKETPLACE_URL_MAP", "{}")
...
_FEATURE_PRODUCT_CODE_MAP = _load_json_map(
    _FEATURE_PRODUCT_CODE_MAP_RAW, "FEATURE_PRODUCT_CODE_MAP"
)
_FEATURE_OFFER_ID_MAP = _load_json_map(
    _FEATURE_OFFER_ID_MAP_RAW, "FEATURE_OFFER_ID_MAP"
)
_FEATURE_MARKETPLACE_URL_MAP = _load_json_map(
    _FEATURE_MARKETPLACE_URL_MAP_RAW, "FEATURE_MARKETPLACE_URL_MAP"
)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    # Resolve product code (real-MP mode requires explicit mapping; simulator
    # mode synthesizes one so subscribe + check find the same row).
    ...
    if _SOURCE_TAG == "marketplace":
        marketplace_url = _FEATURE_MARKETPLACE_URL_MAP.get(feature_id)
        if not marketplace_url:
            ...
            if not _SIMULATOR_ADMIN_ENDPOINT:
                raise SubscribeError(
                    f"No marketplace URL mapped for feature {feature_id!r}. "
                    f"Configure FeaturePlatformMarketplaceUrlMap to include this feature."
                )
