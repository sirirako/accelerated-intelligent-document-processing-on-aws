...
- **Default extraction model is Claude Sonnet 4.6; default confidence model is Claude Haiku 4.5** (both were Amazon Nova variants). Haiku assesses long per-row list confidence reliably where Nova Lite emitted unparseable JSON. Presets that pin an explicit model are unaffected. **Request Anthropic Claude model access in Bedrock before upgrading.**
