...
- **`max_tokens` limit for Sonnet 4.6 / Opus 4.6 was wrongly capped at 64K (true limit is 128K)** — `config_library/model_config_limits.yaml`'s generic `claude-(opus|sonnet|haiku)-4` pattern (first-match-wins) capped Claude Sonnet 4.6 and Opus 4.6 at 64,000 output tokens, so config validation rejected any `max_tokens > 64000` for those models. Both actually support **128,000** output tokens (like Opus 4.7/4.8). Added explicit `claude-sonnet-4-6` and `claude-opus-4-6` patterns (128K) ahead of the catch-all, which now covers only the genuinely-64K 4.x models (Haiku 4.5, Sonnet 4.0/4.5, Opus 4.0/4.1/4.5) — the `:1m` extended-context variants inherit the corrected limits. Fixes spurious `MaxTokensReachedException` headroom on large-table agentic extraction and lets integrated single-shot use the full 128K output budget. Tests: `tests/unit/test_model_utils.py` (`test_claude46_models_return_128k`).

- **Integrated confidence now works on the simple (non-agentic) path** — Previously `confidence.mode: integrated` was only wired into the *agentic* path; on the simple path it was effectively broken: the model was asked to return values + inline confidence but the response's `{"extraction": {...}, "confidence": {...}}` envelope was stored verbatim as `inference_result` (observed on ~34% of RealKIE docs with Haiku — corrupting the extracted data), the standalone Assessment step **still ran anyway** (so integrated was *more* expensive than separate, not less), and confidence was never emitted as `explainability_info`. The simple path now splits the envelope (values → `inference_result`, confidence → the same in-extraction marker the agentic path uses), enriches it with per-field `confidence_threshold` + alerts, reconciles it to full per-cell coverage, grounds geometry, and emits `explainability_info` — which makes the standalone Assessment step auto-skip (the intended single-inference cost win). A flat response with no confidence envelope falls back to the standalone step (no regression). Tests: `tests/unit/extraction/test_nonagentic_integrated.py`.

- **Integrated confidence no longer drops list rows on large tables** — Because integrated confidence rides inline on the extraction inference, a model can under-enumerate a big table (e.g. single-shot scored 76/85 rows), leaving the rest as null-confidence placeholders. Both the batched/separate path (`assess_results_batched`) and the integrated path now **retry only the unscored rows** in focused, bounded (≤2-round, sequential) re-assessment calls and splice the real scores back by index — so large-list confidence coverage reaches 100% instead of relying on placeholders. A retry round that recovers nothing stops early (no infinite loop); retries are best-effort and never fail extraction. Tests: `tests/unit/assessment/test_batched_assessment.py` (`test_missing_rows_are_retried_to_full_coverage`, `test_retry_stops_when_no_progress`).

- **Integrated confidence modes now attach `confidence_threshold` + alerts** — The agentic-integrated path previously emitted raw `{confidence, confidence_reason}` leaves with no `confidence_threshold` and no `confidence_threshold_alerts` (unlike the separate/standalone path). A shared `enrich_assessment_with_thresholds` helper now adds per-field thresholds (from `x-aws-idp-confidence-threshold` or the HITL default) and builds threshold-violation alerts for all integrated paths, so every confidence mode produces one consistent output contract.
...
- **Extraction prompts now respect per-field date/number formats from the Document Schema** — The default extraction prompts (both `task_prompt` and `task_prompt_extraction_with_confidence`) previously hardcoded *"All dates should be in MM/DD/YYYY format"*, which overrode any per-field format an admin specified in the class schema (and could misalign with ground truth during evaluation). The prompts now instruct the model to follow each attribute's own description/format exactly, falling back to the document's rendering only when no format is specified. The RealKIE-FCC-Verified preset's `LineItemStartDate`/`LineItemEndDate` descriptions now specify `MM/DD/YY` to match that dataset's ground truth.

- **Per-field confidence for every list/nested-list cell (not one score per row)** — In integrated confidence mode the agent sometimes emitted a single confidence object for an entire list row, so the UI/HITL (which index confidence per sub-field) showed no confidence for individual line-item columns. The integrated and separate confidence prompts now explicitly require per-column confidence leaves for list rows (mirroring the row's columns, recursively for nested lists), and `_reconcile_assessment_to_data` now normalizes any per-row scalar confidence into per-column leaves as defense-in-depth — so every list-item field gets its own confidence and bounding box regardless of model compliance.

### Added

- **Experimental: single-shot integrated confidence for agentic extraction (hidden `extraction.agentic.integrated_confidence_strategy`)** — On the agentic path, `confidence.mode: integrated` currently produces confidence in a **follow-up tool call** within the extraction turn (`two_step`: extract → `provide_field_assessment` → close, ~3 inferences). A new hidden setting `extraction.agentic.integrated_confidence_strategy` (`two_step` default | `single_shot`) adds a `single_shot` path where the agent emits values **and** per-field confidence in **one combined tool call** (`extraction_with_confidence_tool`), removing the middle inference (~2 inferences per turn/shard). Both strategies write the same `field_assessment` state and produce byte-identical `explainability_info` downstream — only the inference mechanics differ. The setting is **deliberately not surfaced in the config UI**; it exists so operators can A/B the cost/latency-vs-confidence-calibration trade-off (set it via `idp-cli config-upload` on a throwaway config version). Default `two_step` = no behavior change on upgrade. Non-agentic (simple) integrated is already a true single inference and is unaffected. See the extraction README (*Confidence Assessment → integrated-confidence-strategy*) and [docs/assessment.md](docs/assessment.md).

### Changed

- **Docs: unified "Extraction & Confidence" guide + granular-retirement migration note** — `docs/extraction.md`, `docs/assessment.md`, and `docs/assessment-bounding-boxes.md` are consolidated into a single [Extraction & Confidence](docs/extraction-and-confidence.md) guide reflecting v0.6 (confidence and geometry are outputs of extraction, not a separate stage): Simple vs Advanced extraction, the three confidence modes (off/separate/integrated), large-list batching via `list_batch_size`, the four geometry modes (`ocr_only`/`llm_grounded`/`llm`/`off`) with OCR grounding, HITL, confidence thresholds, and large-document guidance. The three old pages are now short stub/redirects so existing links still resolve. A new [Granular Assessment Retirement](docs/migration-granular-retirement.md) migration note documents that `granular.*` keys are a validated no-op (no config edit required), that `list_batch_size` is the single knob, mode guidance for large documents, and that the config format stays v0.6. Docs-site sidebar updated.

- **Granular assessment now defaults OFF (retired)** — With the standalone batching safety net in place, `extraction.confidence.granular.enabled` now defaults to `false` in the system defaults (`base-confidence.yaml`). Large lists are handled by the standalone `list_batch_size` batching, which is cheaper (−78% Bedrock cost measured on a 120-row bank statement) and gives full per-cell confidence **and** geometry (granular's nested `{"items": [...]}` assessment shape produced 0% geometry coverage). **No customer config edit is required:** existing `granular.*` keys still validate and migrate cleanly, but are ignored at runtime by default. A config that still explicitly sets `granular.enabled: true` continues to use the granular service for now, and logs a one-line deprecation warning (*"granular assessment is retired; large lists are batched by the standalone assessment via confidence.list_batch_size"*). The granular service itself is removed in a follow-up PR. See `docs/planning/retire-granular-assessment-plan.md`.

- **Standalone (non-agentic) Assessment now batches large lists on its own** — The standalone Assessment step (`AssessmentService.process_document_section`) previously ran a single confidence inference over the whole section's values. On a large list (e.g. a 120-row bank statement), one call under-enumerates or omits the list, leaving most rows unassessed — historically the reason granular assessment existed for the non-agentic path. The standalone path now uses the same batching + per-column reconciliation the agentic in-shard path already used: it slices the largest oversized list by `extraction.confidence.list_batch_size` (default 25), assesses each chunk **sequentially** with the shared scalars/context, concatenates the per-row assessments in order, and reconciles to full per-cell coverage. Sequential (not a 20-way thread pool) by design — the cost win comes from avoiding a cacheWrite storm. This logic is factored into a shared `idp_common.assessment.batching` module (`assess_results_batched` + `reconcile_assessment_to_data`) imported by both the standalone and agentic paths, so there is now exactly one implementation. This is the safety net that lets granular assessment be retired without regressing large-list customers on simple + separate confidence (see the sequenced retirement plan in `docs/planning/retire-granular-assessment-plan.md`). Unit tests: `tests/unit/assessment/test_batched_assessment.py`.

- **Config UI holistic polish across OCR, Classification, HITL, Summarization, Evaluation, and Chat** — Extends the v0.6 extraction-form treatment to the rest of the configuration editor (schema/renderer only — no config-path or runtime change): (1) **Concise field titles** — every field in these sections now has an explicit `title` (e.g. "OCR backend", "Classification method", "Pages to classify"), so labels are short instead of the full description or an awkward humanized key. (2) **Collision-free ordering** — fractional `order` values (which the renderer's `parseInt` silently truncated to the same integer — e.g. classification's 3.5–3.9 all → 3, and the `classes` name/description fields defaulting to 999) are renumbered to clean integers so field order is explicit, not insertion-order luck. (3) **Disabled features collapse to their toggle** — Summarization, Chat, and Evaluation now hide their model/prompt/sampling knobs when the feature is disabled (via `dependsOn` on the section's `enabled`/`llm_method`), matching how Extraction → Confidence behaves. (4) **`reasoning_effort` is hidden unless an OpenAI model is selected** (it is OpenAI-GPT-5.x-only) across OCR, Classification, Summarization, Evaluation, and Chat. (5) **HITL** review section's confidence-threshold field hides when HITL is off, and its description states the dependency on Confidence Assessment being enabled. (6) **Evaluation → LLM method** renders as a proper always-open named group (previously an unnamed, unordered object). (7) The textarea renderer hint is normalized to `format: textarea` (with `text-area` kept as a back-compat alias), and multi-line prompt fields declare it explicitly instead of relying on the field path containing "prompt".
...
- **AWS AppSync fully removed — replaced by an API Gateway REST API + polling + Lambda response streaming (GovCloud / FedRAMP)** — The UI↔backend transport no longer uses AWS AppSync at all (it is unavailable in GovCloud, not FedRAMP-compliant, and being de-emphasized by AWS). There is **no toggle**. Specifically: (1) UI queries/mutations go to an **API Gateway REST API** with a **Cognito User Pools authorizer**, fronting a single dispatcher Lambda that reuses the existing resolver Lambdas — a shared adapter (`idp_common.api_adapter`) normalizes the API Gateway event to the resolver shape and normalizes the authorizer's `cognito:groups` claim to a list so RBAC is unchanged; DynamoDB-direct operations (discovery/agent jobs, `getDocument`, date queries) are served in-process by the dispatcher. The REST API supports **`ApiGatewayVisibility=PRIVATE`** (VPC-only via an interface endpoint + resource policy) — like the existing headless Jobs API — and an optional **WAFv2 WebACL** on its stage (enabled when `WAFAllowedIPv4Ranges` is set); neither is possible with an HTTP API. (2) the backend writes the TrackingTable directly (`DocumentDynamoDBService`; the document-service factory is DynamoDB-only) and the **5 former status subscriptions are polling** in the UI (reusing the dedup/merge logic), pausing while the browser tab is hidden; (3) the **two chat flows stream via a direct Lambda Function URL** (`AuthType=AWS_IAM` + browser SigV4 with Cognito Identity Pool creds, `InvokeMode=RESPONSE_STREAM` via the AWS Lambda Web Adapter), working identically under CloudFront or ALB hosting; (4) **all `AWS::AppSync::*` resources are deleted** (~150 across the main, nested, pattern, and feature-platform stacks), and the `idp_common.appsync` module is removed. The **Feature Platform** is migrated too: its 6 UI fields route through the REST API dispatcher and its 6 install hooks (register/unregister feature & hooks, apply/remove config preset) are invoked **directly** by feature stacks via re-exported Lambda ARNs instead of AppSync mutations. The misnamed `AppSyncVisibility`/`UsePrivateAppSync` parameters are renamed to **`ApiGatewayVisibility`/`UsePrivateApi`**. Validated end-to-end on a fresh stack (REST API, zero AppSync resources; document processed to COMPLETED; authenticated reads + DDB-direct 200 + 401-on-no-auth confirmed).

- **Extraction config UI polish: single confidence list-batch knob, Advanced settings first, one-line Custom-Prompt ARN, reliable Restore-default** — Four form-level refinements to the Extraction configuration editor (schema/renderer only — no config-path or runtime change): (1) The duplicate **"List batch size"** field is gone — the legacy `extraction.confidence.granular.*` sub-block (a standalone-Assessment ThreadPool tuning group whose single relevant knob duplicated the top-level confidence "List batch size") is no longer surfaced in the UI, leaving one confidence list-batch control; the runtime still reads the granular model-side defaults (the block is slated for full removal, see `GranularAssessmentConfig`). (2) **Advanced extraction settings** (agentic sharding/table-parsing/validation) moves up to just below Extraction mode and above Confidence Assessment, so the mode's own tuning sits next to the mode selector. (3) **Custom Prompt Lambda ARN** moves to the very bottom of the section and renders as a single-line input (via a new `format: single-line` schema hint that opts a field out of the "any path containing 'prompt' → textarea" default), matching the Model Lambda Hook ARN field. (4) The field-level **"Restore default"** control now renders whenever a field differs from the stack default — as a live edit *or* a saved customization (the same signal that paints the yellow "modified" highlight) — and only requires a known default to revert to, so it no longer silently disappears on read-only or the `default` version.

- **Confidence scoring is now a single top-level mode (Off / Separate / Integrated)** — The separate "Enable confidence" toggle + "Integration mode" selector are combined into one **`extraction.confidence.mode`** control (`off` | `separate` | `integrated`, default `separate`), mirroring Geometry mode. `confidence.enabled` is now derived (`enabled = mode != 'off'`) so all existing read-sites keep working; a legacy `assessment.enabled: false` (bool or string) migrates to `mode: off`. In the config form, **Confidence scoring mode** and **Geometry mode** are surfaced at the top of Extraction (right below Extraction mode), so the three key decisions — extraction Simple/Advanced, confidence Off/Separate/Integrated, geometry mode — are all visible at a glance; confidence tuning knobs show only in Separate mode. Also: OpenAI-model descriptions now reference "'advanced' Extraction mode" (not "agentic"), and the field-level "Restore default" control was moved back inline so it renders reliably.

- **Extraction mode: Simple / Advanced (replaces the raw "Agentic Extraction" toggle)** — The config UI now exposes a top-level **Extraction mode** selector — `simple` (single-pass; fast/cheap, short docs) or `advanced` (the robust, sharded engine for large docs / big tables / completeness) — instead of a nested `agentic.enabled` checkbox. A new `extraction.mode` config field is authoritative and the underlying `agentic.enabled` is derived from it (`advanced` → on); a config with no `mode` infers it from `agentic.enabled` for backward compatibility, so all existing behavior and read-sites are unchanged. The former "Agentic Extraction" section is retitled **Advanced extraction settings** (sharding/concurrency, table parsing, schema validation/escalation) and shown only when mode is Advanced. Default is `simple` (unchanged single-pass default).

- **Extraction config layout: key decisions surfaced, feature sections always-open** — The Confidence Assessment, Bounding-box Geometry, and Missing-vs-Blank-Fields sections now render as **always-open groups** (their master toggles — confidence on/off, separate/integrated, geometry mode — are visible without expanding a collapsible box, and can't be hidden behind a collapsed header). Extraction fields are reordered so those feature sections follow the model/prompt/agentic settings, and Image Processing moves out of the top slot. Adds two generic, reusable ConfigBuilder schema hints: `collapsible: false` (render a nested section always-open) and `ghostGroup: true` (visually group sibling fields without changing their config path). Pure UI/config-schema changes — no config-path or runtime changes.

- **Config UI polish for the v0.6 prompt/confidence settings** — Follow-up refinements: the confidence-only prompt now lives in the **Confidence Assessment** section (`extraction.confidence.task_prompt`) and the bounding-box instruction block in the **Bounding-box Geometry** section (`extraction.geometry.task_prompt_bbox`), each shown only where relevant (bbox hidden for `ocr_only`/`off`); the extraction-only prompt is hidden in integrated mode (the extraction+confidence template is used instead); the removed/duplicate empty confidence task-prompt box is gone; the vestigial `confidence.validation_enabled` toggle is removed; the "List batch size" help clarifies it applies to both agentic in-shard and non-agentic standalone separate modes; and "Missing vs Blank Fields" gets a clearer explanation with concise field titles. Migration maps `assessment.task_prompt` → `extraction.confidence.task_prompt`.

- **Editable prompt templates per mode + settings-aware Prompt Preview** — Prompt content is now fully editable config, selected by settings, so the UI Prompt Preview reflects exactly what runs. Four `extraction.*` task-prompt templates: `task_prompt` (extraction only), `task_prompt_extraction_with_confidence` (integrated — one inference emits value + confidence), `task_prompt_confidence` (confidence-only separate pass), and `task_prompt_bbox` (bounding-box instruction block). The engine picks the template by `extraction.confidence.mode` (integrated → extraction+confidence; separate → extraction + confidence-only) and appends the bbox block only when `extraction.geometry.mode` is `llm`/`llm_grounded`. This fixes two gaps: the integrated confidence instructions were a hardcoded Python constant (so the extraction prompt "looked the same" in the UI when switching to integrated), and there was no editable bbox template for the LLM-box geometry modes. The **Prompt Preview** tab now composes and shows the actual extraction and confidence prompts for the current settings, with a note stating which template is active and whether the bbox block is appended. The Configuration form also moves **Confidence Assessment** and **Bounding-box Geometry** adjacent to **Agentic Extraction**, and the field-level "Restore default" button was restored (moved to the FormField secondaryControl slot after it regressed when descriptions were added).

- **Removed the standalone Assessment hook point (postAssessment)** — With confidence folded into extraction there is no separate assessment step to hook. The `postAssessment` pipeline-hook point and the top-level `assessment` config block are removed end to end (SFN workflow `PostAssessmentHook` state, both hook-point maps, the feature-manifest schema enum, the `AssessmentConfig` model + UI schema section). The Assessment Lambda still runs for the non-agentic separate-mode path; migration drops the `assessment` block (its values already map to `extraction.confidence`/`extraction.geometry`/`hitl`).

- **Config UI: concise field names with descriptions as help text** — The schema-driven configuration form previously used each field's full **description as its label**, producing long, sentence-like labels. Labels are now concise (from an optional schema `title`, else a humanized key — e.g. `list_batch_size` → "List batch size", `model_lambda_hook_arn` → "Model Lambda Hook ARN"), and the description renders as lighter help text underneath the field (Cloudscape `FormField` description). Applies to every config section. Added concise `title`s to the v0.6 confidence/geometry/HITL fields ("Enable confidence assessment", "Integration mode", "Geometry mode", "Confidence model", "Enable HITL review", "Confidence threshold").

- **Config v0.6 follow-ups: removed the redundant standalone-assessment enable toggle; unified integrated-mode prompt assembly** — (1) Whether confidence assessment runs is now governed solely by `extraction.confidence.enabled`; the vestigial `assessment.enabled` toggle is dropped (the top-level `assessment` block is now purely the `postAssessment` pipeline-hook carrier). (2) The integrated-mode confidence instructions (appended to the extraction agent's system prompt when `extraction.confidence.mode: integrated`) now come from the same `idp_common.extraction.prompt_assembly` module as separate-mode composition — one source of truth, with the bbox directions included only for the `llm`/`llm_grounded` geometry modes.
...
- **Config format v0.6: confidence and geometry are now outputs of extraction; HITL is its own section (clean break, auto-migrated on read)** — The standalone top-level `assessment` block is retired as the home for per-field confidence scoring. Confidence and bounding-box geometry are reframed as optional **outputs of extraction**, so their settings move under `extraction`:
  ...
  - `assessment.geometry_mode` → **`extraction.geometry.mode`**, with the enum values renamed for clarity: `llm_with_ocr_grounding` → **`llm_grounded`**, `llm_only` → **`llm`**, plus a new **`off`** (no geometry); `ocr_only` (default) is unchanged. The legacy `assessment.ground_geometry_in_ocr: false` maps to `llm`.
  - `assessment.hitl_enabled` / `assessment.default_confidence_threshold` → new top-level **`hitl.{enabled, confidence_threshold}`**.

  A slim top-level `assessment` block survives only as the carrier for the standalone Assessment **step** (`enabled`) and its `postAssessment` pipeline hooks (`postHook`) — unchanged Feature Platform behavior. Existing v0.5 configs are **migrated on read** by a pure, idempotent transform (`config/migrations/v05_to_v06.py`) wired into `IDPConfig` validation and the ConfigurationManager merge path, and stamped `config_format_version: "0.6"`; the runtime only ever sees v0.6, so custom configs and config_library presets keep working with no manual edit. Runtime behavior is unchanged (same models, prompts, geometry results). The Configuration UI is reorganized to match: **Extraction → Bounding-box Geometry** and **Extraction → Confidence Assessment** (with the confidence master enable toggle at the top of its section and inference knobs progressively disclosed / hidden when `mode: integrated`), and a new top-level **Human-in-the-Loop (HITL) Review** section. See [the extraction README](lib/idp_common_pkg/idp_common/extraction/README.md) and [the assessment doc](docs/assessment.md).
- **Default assessment model is now Claude Haiku 4.5 (was Amazon Nova Lite)** — The system-default assessment model (`base-assessment.yaml`) changes from `us.amazon.nova-lite-v1:0` to `us.anthropic.claude-haiku-4-5-20251001-v1:0`. Live testing of in-shard assessment on a 120-row bank statement showed Nova Lite emitting unparseable JSON on long per-row list assessments (only ~45-70 of 120 rows got real confidence; the rest fell back to padded `null`), while Haiku assessed **all 120 rows** reliably. Presets that pin an explicit `assessment.model` (e.g. the GovCloud preset) are unaffected; only deployments inheriting the default change. Request Anthropic Claude model access in Bedrock before upgrading.

- **Extraction Configuration form: clearer layout and conditional fields** — The schema-driven Extraction config editor was reorganized for readability: **Image Processing Settings** moves to the top (matching OCR/Classification), **Agentic Extraction** moves up directly under it and **auto-expands when it is enabled** (so the active configuration isn't hidden behind a collapsed header) via a new generic `expandWhen: {field, value}` schema hint, **Reasoning effort** moves up directly under the model selection and is now **hidden unless an OpenAI model is selected** via a new generic `dependsOn.valuePrefix` hint, and **Missing vs Blank Fields** is pulled out of the jumbled middle to a clean position with its description clarifying it applies to **both standard and agentic** extraction. The extraction fields were also given collision-free integer `order` values (the previous fractional orders like `6.5`/`6.6`/`6.7` were all truncated to `6` by the renderer's `parseInt`, which is what made the middle of the form look scrambled). Finally, the System/Task prompt descriptions now note they can be overridden per document class via the `x-aws-idp-extraction-system-prompt` / `x-aws-idp-extraction-task-prompt` schema extensions in the Document Schema editor. Pure UI/config-schema changes — no extraction behavior change.

### Fixed

- **Config v0.6 migration: importing/upgrading a v0.5 config no longer silently reverts customizations to defaults** — When a v0.5-shaped config (a top-level `assessment` block, or `extraction.assessment_integration`) was **deep-merged onto the v0.6 system defaults BEFORE the v0.5→v0.6 migration ran** — the ordering used by both the deploy-time config load (`update_configuration` Lambda → `merge_config_with_defaults`) and `idp-cli`/SDK/UI **config import** (`ConfigurationManager.handle_update_custom_configuration`) — the migration's "explicit v0.6 keys win over migrated legacy values" rule let the v0.6 **default** values beat the user's migrated legacy customizations. A saved config that pinned, say, `assessment.model`, `inshard_list_batch_size`, `geometry_mode`, and `assessment_integration: integrated` would come back as the defaults (Haiku / 25 / `ocr_only` / `separate`) — a **silent data-loss on in-place stack update and on config import from older files**. Both paths now **migrate the incoming config/delta to v0.6 first, then merge**, so it is a clean v0.6-over-v0.6 deep-merge where user values win. `merge_config_with_defaults` gained a `migrate: bool = True` parameter and migrates internally as the single choke point (the migration is idempotent, so it is a no-op for already-v0.6 input); `handle_update_custom_configuration` migrates the delta once before its saveAsVersion/saveAsDefault/normal-update merges. Regression tests added (`test_merge_migration_order.py`). The `IDPConfig` validate-time migration remains as the last-line safety net.

- **OCR geometry grounding now bridges format differences (dates, currency, phone)** — With OCR-only geometry, a field whose extracted value is canonicalized to a schema format that differs from the document's rendering (e.g. `statement_end_date` extracted as `2022-04-04` but printed as `04/04/2022`; `1234.00` vs `$1,234.00`; `+15551234567` vs `(555) 123-4567`) previously failed value-text matching and got **no bounding box**. Matching is now format-aware via three precision-preserving layers: **format variants** (the value re-rendered in common surface forms), **type-aware equality** (value and OCR line parsed as the same logical date/number/phone and compared — robust to any rendering, reusing the evaluation module's `normalize_numeric`), and a **character-level Levenshtein** last resort for OCR noise (reusing `evaluation.comparator.fuzz_score`, threshold 0.8). The field's logical type comes from its JSON-Schema `type`/`format` when present, else is inferred from the value (number-before-date and phone-before-number disambiguation so a bare integer isn't read as a date and a phone isn't read as a number). Format-bridged matches are tagged `geometry_source: "ocr-normalized"` and Levenshtein near-misses `"ocr-fuzzy"`, distinct from exact `"ocr"` hits. Threaded through the standalone Assessment step, granular assessment, and the in-shard assessment path. See [the assessment doc](docs/assessment.md#geometry-source-ocr-only-default-vs-llm-estimated).

- **Processing Report: impossible table-parsing stats (500% parse-success rate, 496% confidence) + outcome-first redesign** — On a sharded agentic extraction the per-shard `_table_parsing_stats` (parse-success rate, average OCR confidence) rode the *additive* metering channel in `_accumulate_metering`, so they were summed across shards like token counts — 5 shards each at ~100% became "500%" / "496%". The merge now special-cases `_table_parsing_stats`: counts (`tables_parsed`, `rows_parsed`, `rows_mapped`, `invocation_count`) still sum, but `parse_success_rate`/`avg_confidence` combine as **row-weighted averages** so the merged value stays a real 0-1 rate / 0-100 confidence, and a display-time `_normalize_table_parsing_stats` guard clamps both ranges as defense-in-depth. The **Processing Report** UI is also redesigned outcome-first: a top-line verdict (Completed / Completed with N items to review / Failed) with the authoritative count of rows + fields actually extracted (computed from the inference result, not the pre-flight OCR estimate), a plain-language list of issues to review, and a collapsed "Details & diagnostics" section. Internal jargon is replaced with plain language — `MANDATORY`/`OPTIONAL` tool-recommendation strings become "the deterministic table parser was used / was not used / was not needed", and the report now explains that the OCR "tables detected / estimated rows" figure is a pre-extraction estimate distinct from the authoritative extracted-row count.

### Added

- **OCR-only geometry is now the default for assessment (cheaper, more accurate, no hallucinated boxes)** — A new `assessment.geometry_mode` setting (default **`"ocr_only"`**) stops asking the model for bounding boxes entirely and instead derives each field's geometry by matching the extracted value text against real OCR lines in `pageData.json`. This is cheaper (no bbox coordinate tokens in the assessment/extraction response) and more accurate than LLM-estimated boxes, which models routinely hallucinate. Repeated values (e.g. the same amount on several table rows) are disambiguated by **row order** — the i-th assessed list item maps to the i-th occurrence of that value in reading order (page, then top, then left) — so confidence boxes land on the right row without needing an LLM reference box. A field with no OCR match simply gets no geometry (geometry is advisory). The previous behavior is available as `"llm_with_ocr_grounding"` (model emits boxes, OCR grounding refines them, LLM box disambiguates repeats), and `"llm_only"` keeps raw model boxes with no grounding. The legacy `assessment.ground_geometry_in_ocr` flag is still honored (`false` → `llm_only`); `geometry_mode` takes precedence. In `ocr_only`, the assessment/extraction prompts drop the bbox/spatial-localization instructions (the integrated `provide_field_assessment` tool is told not to emit boxes), and any stray model-provided box is ignored — so no hallucinated coordinates can leak into `explainability_info`. The grounding helpers (`match_value_to_geometry`, `ground_assessment_geometry`) gain an `occurrence_index`/`geometry_mode` parameter; the standalone Assessment step, the in-shard assessment path (separate mode), and the inline integrated mode all route through it. Output shape is unchanged. See [the assessment doc](docs/assessment.md#geometry-source-ocr-only-default-vs-llm-estimated).

- **Integrated assessment: confidence/bbox scoring runs inside agentic extraction shards (opt-in, no behavior change by default)** — A new `extraction.assessment_integration` setting (`"separate"` default, or `"integrated"`) controls *where* per-field confidence and bounding boxes are produced. With the default `"separate"` + agentic extraction, a **second assessment inference now runs inside each extraction shard** over that shard's pages and extracted values, so assessment inherits the same per-shard scaling as extraction (a 200-page section never assesses in one oversized call — the failure mode a single post-merge assessment would re-introduce). Per-shard assessments are **collated on merge** (per-field, page-ordered for list items so `explainability_info` lines up with the merged `inference_result`, first-shard-wins for scalars, alerts unioned), and each list field's per-item assessment is **reconciled to exactly the extracted row count** (truncate extras / pad shortfalls with a neutral `confidence: null` entry) so confidence is never misattributed to the wrong row when the assessment LLM under/over-counts rows — validated live where a 120-row table came back with only 44 row assessments. Collated assessment is grounded once in real OCR geometry over the whole section, and emitted as `explainability_info` — **byte-for-byte the same output contract** the standalone Assessment step produces (`explainability_info[0]` + `section.confidence_threshold_alerts`), so HITL, the UI confidence display, evaluation, and reporting consume it unchanged. The downstream standalone Assessment step **auto-bypasses** (its existing "intelligent skip" returns immediately when `explainability_info` is already present), so there is no duplicate cost and **no state-machine change**; document status stays `EXTRACTING`. This reuses the **same** assessment logic, not a reimplementation: `AssessmentService.assess_results(...)` was extracted as a pure, S3-free core that both the standalone step and the in-shard path call, and OCR grounding reuses `assessment.ocr_grounding.ground_assessment_geometry`. Non-agentic extraction with `"separate"` is unchanged (flows to the standalone step). The in-shard pass is wired through the same runtime primitive both backends share (`extract_one_shard` gains an injected, strands-free `assess_runner`; the per-shard assessment is persisted alongside the extracted fields so SFN/asyncio resume covers it too). UI: a new **Assessment integration** dropdown on the Extraction config form, plus a generic cross-section `hideWhen` schema hint that greys out the now-unused Assessment inference fields when `"integrated"` is selected. The **`"integrated"`** mode is also implemented: the extraction agent emits per-field confidence/bbox **inline in its own inference** (the document is already in its cached context — no second model pass) by calling a new `provide_field_assessment` tool as its final step, and that inline result rides the **same** collation → reconcile → grounding → emit path as `separate` (so the output contract is identical); the standard non-agentic path passes the same flag to its single inference. Defaults preserve existing behavior on upgrade. See [the extraction README](lib/idp_common_pkg/idp_common/extraction/README.md#integrated-assessment-in-shard-confidence--bounding-boxes) and [the assessment doc](docs/assessment.md).

- **Pluggable `ExtractionRuntime` for sharded agentic extraction — in-process and nested Step Functions Distributed Map over the same primitives** — Sharded extraction is factored into runtime-agnostic primitives in a new strands-free module `idp_common.extraction.runtime` (the single source of truth): `extract_one_shard` (runs ONE shard via an injected agent runner, with **idempotent per-shard S3 persistence** — a completed shard is loaded and skipped instead of re-inferring), `merge_shard_results`/`merge_shard_dicts` (page-ordered list concatenation + first-non-null scalars), and a `ShardPersistence` protocol (`S3ShardPersistence` keyed at `checkpoints/{execution_arn}/{section_id}/shards/shard_{start}_{end}.json`). Two schedulers drive the identical primitives behind an `ExtractionRuntime` interface: **`InProcessRuntime`** (default — `asyncio.gather` + semaphore, used by notebooks / CLI / a single Lambda; standalone sharding works with no Step Functions dependency) and **`StepFunctionsRuntime`** (production — a nested SFN **Distributed Map** where each iteration is a thin shard Lambda calling `extract_one_shard` and a following merge state calls `merge_shard_results`, so SFN's native per-iteration retry re-runs **only the failed/incomplete shards** while completed shards load from S3 — no custom near-timeout/reentry code). The runtime is selected by `extraction.agentic.runtime` (`in_process` default, or `step_functions`); `ExtractionService.process_document_section()` is unchanged for standalone use and `concurrent_structured_output_async` is now a thin shim over the runtime (identical observable behavior, single-pass unchanged). Additionally, the in-process section Lambda now wires `S3ShardPersistence` so an SFN `ExtractionStep` retry of a timed-out section resumes only the incomplete shards. New `ShardRuntimeFunction` (reuses the extraction container image via a different `ImageConfig.Command`, no extra Docker build) and an `ExtractionPlanStep → ShardModeChoice → (Distributed Map → ExtractionMergeStep) | ExtractionStep` branch in `workflow.asl.json`. See [the extraction README](lib/idp_common_pkg/idp_common/extraction/README.md) and `notebooks/misc/standalone_sharded_extraction_demo.py`.

### Changed

- **Agentic sharded extraction now engages by default on large documents — no per-config tuning** — The default `extraction.agentic.shard_token_budget` is lowered from **40,000 to 8,000**, and a new `extraction.agentic.max_pages_per_shard` page ceiling (default **5**, `0` disables) bounds each shard by page count in addition to tokens. The old 40,000 budget was high enough that even a ~25-page dense table fit a single shard, so sharding silently did *not* engage and one agent had to emit the whole giant table in one Bedrock call → read timeout. A shard now closes when *either* the token budget *or* the page ceiling is hit, so a large document shards reliably even when its OCR text is unusually compact (tokens **and** pages both bound a shard). Single-pass extraction (`max_concurrent_batches: 1`, the default) is unchanged; raise `shard_token_budget` for large-context (`:1m`) models if you want fewer, larger shards. Validated at scale on fabricated 100- and 200-page single- and multi-table PDFs in both table-parsing and pure-sharded-LLM modes (row counts exact, sequential indices contiguous after merge, no read-timeout / `ContextWindowOverflow`). See [the extraction README](lib/idp_common_pkg/idp_common/extraction/README.md).

- **Agentic sharded extraction: drop phantom trailing rows on merge** — A shard's model sometimes appends "continuation" rows past a table's real end — a lone sequential index (e.g. `RowID`) with every other column null. The shard merge now drops such phantom rows (a multi-column row object with fewer than two populated fields), so a sharded large-table extraction returns exactly the real rows. Sparse 1-2 field row schemas and scalar list elements are unaffected (the filter only applies to dict items with >= 3 fields).

- **Agentic sharded extraction: header context no longer duplicates page-1 table rows** — When sharding a section, page 1's text is prepended to later shards as "DOCUMENT HEADER" context so column headers survive the split. Previously the *entire* page-1 text (including its data rows) was prepended, so the deterministic table parser re-emitted page-1's rows in every later shard, duplicating them after merge. The header context now truncates at the Markdown table separator line (`|---|`), keeping the column header but dropping page-1 data rows (which shard 0 already covers); non-table pages fall back to a small line cap.

- **Agentic extraction: tolerate stringified numeric schema constraints (`minItems`, `minimum`, ...)** — The Configuration table stores numeric JSON-Schema constraint keywords as strings, so a class schema with an array `minItems` (the recommended way to flag a large table for completeness) crashed two extraction paths with `TypeError: '>'/'<' not supported between instances of 'str' and 'int'`: (1) shard planning (`_analyze_schema_for_table_requirements`, `minItems > 50`) and (2) the schema-constraint validation step (jsonschema comparing `len(instance) < minItems`). Both now coerce the numeric keywords (`minItems`/`maxItems`/`minimum`/`maximum`/`minLength`/`maxLength`/...) back to numbers before comparison.

- **Agentic extraction: cap page-images per agent + retry Bedrock read timeouts** — New `extraction.agentic.max_images_per_agent` (default **20**, `0` = unlimited) caps how many page images are attached to a single agent invocation when the task prompt uses `{DOCUMENT_IMAGE}`; beyond the cap only the first N are attached (the agent still has full OCR text and can fetch pages via the `view_image` tool). This prevents the oversized-first-turn Bedrock **read timeout** seen on long documents. Bedrock read/connect timeouts are now retried instead of surfacing as a fatal `EventLoopException`.
