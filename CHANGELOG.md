...
- **Default extraction and confidence models are now Claude Sonnet 5** (both were Amazon Nova variants; interim v0.6 defaults were Sonnet 4.6 for extraction and Haiku 4.5 for confidence). Sonnet 5 gives the best extraction quality and assesses long per-row list confidence reliably where Nova Lite emitted unparseable JSON. Presets that pin an explicit model are unaffected. **Request Anthropic Claude Sonnet 5 access in Bedrock before upgrading.**
