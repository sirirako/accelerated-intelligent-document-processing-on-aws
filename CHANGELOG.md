...
- **Feature Platform — installable features ("marketplace of extensions")** — The main stack can now host *installable features*: independent CloudFormation stacks that an admin launches into the same account, which then upload their UI bundle into the host's `WebUIBucket` and register themselves in a new `InstalledFeatures` DynamoDB table. Registered features appear as nav items in the existing web UI, each rendering its own UMD-loaded React bundle via a shared host-globals contract (`window.IdpFeatures.register`). Backed by a nested `FeaturePlatformStack` (under `feature-platform/main-stack-extensions/`) exposing AppSync operations (`listCatalogFeatures`, `listInstalledFeatures`, `checkFeatureEntitlement`, `getFeatureLaunchUrl`, `subscribe/unsubscribeFeature`, `registerFeature`, `registerFeatureHooks`). Two feature kinds are supported, distinguished by a `source` field: **OSS features** (bundled, open-source — the template is published alongside the accelerator in the same public artifacts bucket as the main template and is installed via a plain CloudFormation quick-create URL, no presign) and **Marketplace features** (closed-source, *future* — the template lives in a private seller bucket and is handed out as a presigned `GetObject` URL only after `GetEntitlements` confirms an active AWS Marketplace subscription; no marketplace features ship today). Discovery is **manifest-driven**: `idp-cli publish` writes a single `catalog.json` (merging the bundled OSS features with the curated `config_library/extensions-marketplace.yaml`), which is deployed into the stack's own **ConfigurationBucket** and read at runtime with one `GetObject` — the host never performs `ListObjectsV2` and the deployed stack does not depend on the artifacts bucket for the catalog. **On by default** (`EnableFeaturePlatform=true`) in **auto-subscribe** mode: with no `FeaturePlatformSimulatorEndpoint` set, every catalog feature is treated as subscribed and the UI goes straight to Install (no entitlement calls). Set `FeaturePlatformSimulatorEndpoint` to bolt on a marketplace-simulator or real AWS Marketplace endpoint (no rebuild), or `EnableFeaturePlatform=false` to remove the platform entirely. Authors scaffold new features from the bundled `feature-platform/feature-template/` via `idp-feature-cli init`. The UI surfaces installed + catalog features in an "Extensions" nav section with an info panel, and each feature can expose a "Learn more" link via an optional `docsUrl` (OSS features point at a docs-site page under `docs/extensions/`; marketplace features fall back to their listing URL). See the new [Feature Platform](docs/feature-platform.md) and [Developer Guide](docs/feature-platform-developer-guide.md).

- **Feature Platform — config presets + Health Insurance Review advanced sample** — Completed the manifest's `configPreset` contract host-side: new IAM-auth AppSync mutations `applyFeatureConfigPreset` / `removeFeatureConfigPreset` (resolver in `feature-platform/main-stack-extensions/lambdas/apply_feature_config_preset/`) let a feature ship a bundled accelerator configuration and apply it **at install as a new, non-active config version** (`<featureId>-v<version>`) for an admin to activate — installation never changes the active configuration; uninstall removes the feature's preset versions but preserves one that is currently active. The main stack now also re-exports `<MainStackName>-OutputBucketName`, `-WorkingBucketName`, and `-DiscoveryBucketName` for features that read processing results, decompress pipeline-hook document payloads, or drive the host's Rules Discovery flow. Shipped a second bundled OSS sample, **Sample: Health Insurance Review** (`feature-platform/sample-health-insurance-review/`, featureId `sample-health-insurance-review`), demonstrating a health-insurance-claims vertical end-to-end: it applies a prior-auth rule-validation config preset, registers a `postRuleValidation` pipeline hook that derives a **deterministic** claim status (Clean Claim / Review Required / Insufficient Documentation) into its own DynamoDB table, exposes a multi-route Cognito-auth feature API, and renders a two-tab UI (a Claims Dashboard with per-rule drill-down, and a Rules Discovery tab that drives the host's `uploadDiscoveryDocument` / `listDiscoveryJobs` / `getConfigVersion` operations directly via the shared Amplify instance — the host now also exposes `aws-amplify/api` as the `awsAmplifyApi` feature global). The existing `docs-by-status` sample (display name **Sample: Document Status (feature add-on)**) remains the minimal contract reference. See [Sample: Health Insurance Review](docs/extensions/sample-health-insurance-review.md) and the updated [Feature Platform](docs/feature-platform.md) / [Developer Guide](docs/feature-platform-developer-guide.md).
  - **`idp-feature-cli deploy` — publish + install one extension from source into a running host stack.** The per-extension analogue of `idp-cli deploy`: it builds and publishes a single feature, then create-or-updates that feature's CloudFormation stack against an already-running IDP main stack (defaulting to the same stack name a console install uses, so it upgrades in place rather than creating a duplicate). This is the inner-loop developer command for iterating one extension against a live deployment, complementing `idp-feature-cli publish` (artifacts only) and `deploy-pack` (a self-contained pack with its own host stack). Requires the AWS SAM CLI. See the [Developer Guide](docs/feature-platform-developer-guide.md).
  - **Version-free extension publish layout.** Both publishers (`idp-cli publish` and `idp-feature-cli publish`/`deploy`) now write each extension's template to a version-free key (`<prefix>/extensions/<id>/template.yaml`, newest publish overwrites) with versioned artifacts — UI bundle, config preset, manifest — under `<base>/<version>/`, mirroring how the main template is published. The extension version and artifact base prefix are baked into the template at publish time rather than passed as CloudFormation parameters (the console "Update stack" wizard drops parameters on a template change), and the publish path uploads the `configPreset` file so a feature declaring one installs cleanly.
...
- **Feature Platform pipeline hooks were silently dropped from config versions (Claims Dashboard / hooks never fired)** — A `postRuleValidation` (or any post-step) hook stored inline under a config version's `<step>.postHook` was lost whenever that config round-tripped through `IDPConfig` validation — i.e. on **Save as Version**, `updateConfiguration`, and the sparse-config **auto-migration** in `ConfigurationManager.get_merged_configuration()`. The step config models (`OCRConfig`, `ClassificationConfig`, `ExtractionConfig`, `AssessmentConfig`, `RuleValidationConfig`, `SummarizationConfig`) inherit Pydantic's default `extra="ignore"` and did not declare a `postHook` field, so the hook list was stripped on validation; the pipeline-hooks dispatcher then read an empty list and never invoked the feature's hook Lambda (symptom: the Health Insurance Review sample processed documents and produced rule-validation results, but the Claims Dashboard stayed empty). Fixed by adding a typed `PipelineHook` model and a `postHook: List[PipelineHook]` field to all six hookable step configs in `lib/idp_common_pkg/idp_common/config/models.py`, so hooks survive validation on every read/write path. Relatedly, `applyFeatureConfigPreset` no longer writes the `_config_format: "full"` marker onto a sparse preset (that marker skipped the defaults merge and caused "No system_prompt found in classification configuration").
