...
[Extraction & Confidence guide](docs/extraction-and-confidence.md) and the
[Granular Assessment Retirement note](docs/migration-granular-retirement.md).

### Changed

- **Config format v0.6: confidence & geometry are outputs of extraction; HITL is its own section (auto-migrated on read).** The former top-level `assessment` block is retired as the home for confidence scoring. Settings move under `extraction`, and existing v0.5 configs are migrated automatically by an idempotent transform (`config/migrations/v05_to_v06.py`) at both config-import and deploy-time-merge — the runtime only ever sees v0.6, so custom configs and presets keep working unchanged:
  - `extraction.assessment_integration` → **`extraction.confidence.mode`** — a single control: `off` | `separate` (default) | `integrated`.
  ...
  - `assessment.geometry_mode` → **`extraction.geometry.mode`**, enum values renamed for clarity: `ocr_only` (default) | `llm_grounded` (was `llm_with_ocr_grounding`) | `llm` (was `llm_only`) | `off` (new).
  - `assessment.hitl_enabled` / `assessment.default_confidence_threshold` → new top-level **`hitl.{enabled, confidence_threshold}`**.

  A slim `assessment` block survives only as the standalone Assessment-step / pipeline-hook carrier. The config UI is reorganized to match: a top-level **Extraction mode** selector (Simple / Advanced), **Confidence scoring mode** and **Geometry mode** surfaced at the top of Extraction, and a new **Human-in-the-Loop (HITL) Review** section — plus concise field titles (descriptions become help text) and progressive disclosure across every section.

- **Default extraction model is now Claude Sonnet 5; default confidence model is Amazon Nova Lite** (extraction was Nova, then interim Sonnet 4.6; confidence was Nova, then interim Haiku 4.5). Sonnet 5 gives the best extraction quality. Confidence stays on **Nova Lite** because it is far cheaper for the assessment pass: a live A/B on the OCR benchmark (10 docs, identical prompt) measured **~$0.0011/doc on Nova Lite vs ~$0.145/doc on Sonnet 5 — ~130×** — driven by Sonnet's higher per-token rate *and* ~2× more (verbose) output tokens, which dominate assessment cost. Both models use the confidence prompt's `<<CACHEPOINT>>` prefix caching, so caching is not the differentiator; the model choice is. Presets that pin an explicit model are unaffected. **Request Anthropic Claude Sonnet 5 access in Bedrock before upgrading.**

- **Confidence prompts emit `confidence_reason` only for lower-confidence leaves (below 0.9), capped at ~12 words.** A high-confidence value now returns just `{"confidence": <score>}`. Because output tokens dominate assessment cost, this cuts confidence output substantially with no change to scores, thresholds, or alerts (the free-text reason is human-facing metadata, and threshold enrichment spreads whatever leaf keys are present). Applies to both the separate-pass and integrated confidence prompts. To restore a reason on every field, widen the 0.9 threshold in the confidence `task_prompt`.

- **OCR-only geometry is the default** (`geometry.mode: ocr_only`) — field bounding boxes are derived by matching extracted values to real OCR lines in `pageData.json` instead of asking the model for coordinates. Cheaper (no bbox tokens) and more accurate (no hallucinated boxes); repeated values are disambiguated by row order. Matching is **format-aware** — dates, currency, and phone numbers still ground even when the extracted value is canonicalized to a different surface form than the document's rendering. LLM-box modes (`llm`, `llm_grounded`) remain available.

- **Advanced (agentic) extraction shards large documents reliably and runs on a Step Functions Distributed Map.** Sharding now engages by default on large docs (token *and* page-count bounds per shard), each shard persists idempotently to S3 so SFN retries re-run only failed shards, and confidence assessment runs **inside each shard** (scaling with extraction) — collated, reconciled to the exact row count, grounded once, and emitted as the same `explainability_info` contract. Includes fixes for phantom trailing rows, duplicated page-1 header rows, stringified schema constraints, and oversized-first-turn read timeouts.

- **Extraction & confidence always request the model's maximum output; `max_tokens` is no longer a config knob for them.** The Bedrock Converse default-when-`maxTokens`-omitted is a small truncating value (measured 4096 for Claude, 2000 for Nova), so the client now sets it explicitly from the per-model limits (`config_library/model_config_limits.yaml`, also bundled into the `idp_common` package so it resolves at Lambda runtime, not only from the repo root at publish time). The `extraction.max_tokens` and `extraction.confidence.max_tokens` fields are removed (leftover values are ignored — no migration needed); completeness matters more than an output cap for these passes. Fixes agentic extraction failing with `MaxTokensReachedException` on Claude Sonnet 5, whose true 128K limit a stale hard-coded fallback had capped at 4096 — the agentic path now resolves the limit from the same single source as the rest of the system. If a request still exceeds the model limit, the client parses the real cap from Bedrock's `ValidationException` and retries once. `classification`/`summarization` keep their `max_tokens` knob.

- **AWS AppSync fully removed — replaced by an API Gateway REST API + polling + Lambda response streaming.** The UI↔backend transport no longer uses AppSync (unavailable in GovCloud, not FedRAMP-compliant). UI queries/mutations go through a REST API with a Cognito authorizer (supporting `ApiGatewayVisibility=PRIVATE` and an optional WAF WebACL); status updates poll; the two chat flows stream via a direct Lambda Function URL. All `AWS::AppSync::*` resources are deleted and the Feature Platform is migrated. Parameters `AppSyncVisibility`/`UsePrivateAppSync` are renamed to **`ApiGatewayVisibility`/`UsePrivateApi`**.

### Removed

- **Granular assessment is retired and its service deleted.** The `GranularAssessmentService`, the `granular_service.py` module, and the `extraction.confidence.granular` config field no longer exist. Large-list assessment is now handled by **standalone large-list batching** (`extraction.confidence.list_batch_size`, default 25): the model scores each large list in sequential batches, then any rows it drops are re-scored by a bounded missing-row retry, so per-cell coverage reaches 100%. On a 120-row bank statement this was **~78% cheaper** than granular with equal accuracy and full per-cell confidence + geometry (granular's nested output shape produced 0% geometry). **Migration is a no-op:** leftover `granular.*` keys still validate and are ignored — no config edit required. The standalone Assessment step's `postAssessment` pipeline-hook point is also removed. See [Granular Assessment Retirement](docs/migration-granular-retirement.md).

- **`idp_common` code-level breaking changes (affects custom scripts and notebooks, not just YAML config).** The config auto-migration keeps *runtime* behavior working, but code that touches the removed Python symbols breaks at import/attribute time:
  ...
  - **`IDPConfig` no longer has a top-level `assessment` attribute** — `config.assessment.*` raises `AttributeError`. Read confidence/geometry settings from **`config.extraction.confidence.*`** and **`config.extraction.geometry.*`**, and HITL settings from **`config.hitl.*`**. (A v0.5-shaped `{"assessment": {...}}` *dict* still works — it is migrated when passed to `IDPConfig(**cfg)` / `AssessmentService(config=cfg)` — but the *typed* attribute is gone.)
  - **`ExtractionService` and `AssessmentService` public method signatures are unchanged** (`__init__(region, config)`, `process_document_section(document, section_id[, checkpoint_data])`, `assess_document(document)`), so scripts using only those keep working. New additive exports: the runtime-agnostic sharding primitives in `idp_common.extraction.runtime` (`ExtractionRuntime`, `S3ShardPersistence`, `extract_one_shard`, `select_runtime`, …) and `assessment.batching` / `assessment.ocr_grounding`.
  - **Notebooks:** the `step4_assessment_granular.ipynb` notebooks (in `notebooks/examples/` and `notebooks/usecase-specific-examples/multi-page-bank-statement/`) are retired — granular no longer exists, so `step4_assessment.ipynb` is now the single confidence/assessment notebook.

### Added

- **Reasoning effort is now configurable for reasoning-capable Claude models (extraction defaults to `low`).** `reasoning_effort` previously applied only to OpenAI GPT-5.x (Responses API); it now also drives Claude's `output_config.effort` on the Bedrock Converse path for effort-capable models (Sonnet 5, Sonnet 4.6, Opus 4.5–4.8, Fable 5 — accepting `low`/`medium`/`high`/`xhigh`/`max`; Sonnet 4.5, Haiku 4.5, and Nova have no effort control and are unaffected). Wired through both the standard and agentic (Strands) extraction paths. The config UI shows the **Reasoning effort** selector only when the section's selected model supports it (capability-gated, replacing the OpenAI-prefix gate). **Extraction defaults to `low`:** a full effort sweep (5 extraction methods × {low, medium, high, xhigh} × 2 datasets × 20 docs) found higher effort adds output-token cost with negligible extraction-accuracy gain, so `low` keeps the Sonnet 5 default affordable; raise it per-config for genuinely reasoning-heavy documents.

- **New developer skill: `live-eval-and-cost.md`** — documents the live-stack workflow for benchmark A/B tests (model/prompt/config), in-place upgrade testing, and reading accuracy/cost/confidence from a run (metering → `pricing.yaml`, `explainability_info` confidence distributions), plus the build/deploy gotchas (publish `--clean-build` for data-only edits, `pricing.yaml` empty-units rollback deadlock) and prompt-cache/model cost facts (Nova Lite for confidence, ~2048-token Anthropic cache floor, `reasoningContent` parsing).
...
- **`idp-cli config-validate --emit-migrated <path>`** — writes a config migrated to the current format (v0.5 → v0.6) to a file so you can review the v0.6-shaped result before importing.
...
- **Confidence assessment no longer silently loses scores + bounding boxes when the model truncates its output.** When per-row assessment output is large — most visibly with `geometry.mode: llm`, which asks the model for a bounding box on every cell — a batch could exceed a small-cap confidence model's max output tokens (e.g. Amazon Nova Lite's 10,000). The truncated response is unparseable JSON, and the service silently fell back to a default `0.5` for every scalar and left most list rows as null-confidence placeholders (observed: 50 of 68 transaction rows on a lending-package bank statement, with no error surfaced). The assessment core now **detects truncation** (`stopReason == "max_tokens"` for Bedrock Converse, plus the OpenAI-Responses `incomplete`/`max_output_tokens` status; exposed as `AssessmentCoreResult.truncated`) and the large-list batcher **recursively halves any truncated slice and re-assesses it until it fits** — instead of accepting the placeholder — in both the initial batch loop and every missing-row retry. This protects all confidence paths uniformly: the standalone Assessment step (`separate`, incl. **simple mode** — the granular-assessment replacement), the agentic single-agent and sharded in-shard passes, and the simple/agentic `integrated` inline-confidence retry. The activity is surfaced for visibility only when a run actually had to shrink batches: `metadata.assessment_batch_split_stats` on the section result (`truncated_calls`, `splits`, `min_batch_size_used`, `rows_recovered_by_retry`, `unrecoverable_rows`) and an `⚠ Assessment Batch Splitting` block in the agentic processing report. If rows remain unscored at a single-row batch, the durable fix is to reduce per-row output (e.g. `geometry.mode: ocr_only`, the default). Unit tests added.
...
- **Reasoning models (Claude Sonnet 5, Sonnet/Opus 4.6+, any model with extended thinking) returned empty extractions/classifications.** These models emit one or more `reasoningContent` blocks *before* the answer `text` block, but the response parsers read only `content[0].get("text")` — which is the reasoning block, yielding an empty string. Every extraction, classification, assessment, and summarization silently failed (`extraction_parsing_failed`, 0% accuracy) whenever the model chose to think. The parsers now concatenate all `text` blocks and skip `reasoningContent`. Affected: `bedrock/client.extract_text_from_response`, `classification/service` (page + holistic), `summarization/service`.
...
- **Simple-path `integrated` confidence no longer silently pays for a redundant second assessment pass.** The integrated extraction+confidence prompt instructed the model to report confidence by calling a `provide_field_assessment` / `extraction_with_confidence` **tool** — which only exists on the agentic path. On the Simple (non-agentic) path there is no such tool, so the model emitted confidence as a `field_assessment` **sibling** key instead. `ExtractionService._split_inline_confidence` recognized only the `{"extraction": {...}, "confidence": {...}}` envelope, missed the sibling, wrote no `explainability_info`, and the standalone Assessment step then ran as a fallback — so `integrated` mode (whose entire purpose is one inference) quietly paid for **two** Bedrock calls, and the stray `field_assessment` blob leaked into `inference_result`. Fixed on both sides: the prompt now tells the non-tool case to emit the `{extraction, confidence}` envelope directly, and the splitter also lifts a `field_assessment`/`confidence` sibling (guarded so a real field of that name is not misread), stripping it from `inference_result` and promoting it to `explainability_info` so the standalone step correctly auto-skips. Accuracy A/B did not catch this because the fallback still produced confidence and left extraction accuracy unchanged — only the metering (two Bedrock calls) revealed it. Unit tests added.
