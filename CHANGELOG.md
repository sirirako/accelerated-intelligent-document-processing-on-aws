...
- **Federated sign-in lockout after first login (Cognito `email` immutable conflict)** — `UserPool.Schema` declared `email` as `Required: true, Mutable: false` while `ExternalIdentityProvider.AttributeMapping` mapped `email → email`. Cognito rewrites mapped attributes on every federated login, so the second sign-in by any federated user (Okta/SAML/OIDC) was rejected with `user.email: Attribute cannot be updated`, producing a redirect loop back to the login page. Schema flag changed to `Mutable: true` so per-login attribute writes succeed; `Required: true` retained. Cognito-native sign-up/sign-in flows unchanged. **Note:** schema flags are fixed at User Pool creation — existing deployments must redeploy with a fresh User Pool to pick up the fix; new deployments are fixed automatically. See `docs/external-idp.md` for migration guidance.
