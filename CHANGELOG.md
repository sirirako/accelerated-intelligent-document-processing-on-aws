...
- **Processing Report surfaces extraction model, validation, and population metadata** — The section Visual Editor's **Processing Report** tab now renders the audit metadata added with agentic schema-validation/escalation and the completeness heuristic, which it previously ignored. The **Extraction Overview** now shows the **model** that ran the section (with a "(per-class override)" note when `x-aws-idp-extraction-model` applied). A new **Schema Validation & Escalation** panel shows the validation result (valid / N violations), `fail_action`, whether format checks ran, and — when escalation fired — whether it **resolved** the failures, the escalation model, the re-extracted field subset, and the before→after error counts, plus an expandable list of each violation (path / validator / message). A new **Field Population** panel shows the completeness heuristic (`populated/defined` fields, threshold, below-threshold status, and an expandable list of empty fields). The top "Extraction Issues Detected" banner now also fires on a failed validation or a below-threshold population ratio, so silent extraction loss is visible at a glance. Purely additive UI (reads existing `metadata.*`); sections produced before these fields existed render exactly as before.
- **Assessment bounding boxes grounded in real OCR geometry (`pageData.json`)** — The Assessment service previously derived each field's `explainability_info` bounding box purely from the **LLM's estimated** coordinates (returned on a 0–1000 scale, rescaled to 0–1). It now performs a **post-LLM, server-side enrichment pass** that matches each extracted attribute *value* against the real OCR lines in the consolidated `pageData.json` artifact (already normalized 0–1) and, on a confident match, **replaces the LLM-estimated box with the real OCR box** and records provenance. Matching is tiered (exact → value-in-line → multi-line span union → line fragment → token-overlap fuzzy ≥ 0.6). Crucially, when the same value appears on multiple rows (e.g. an identical transaction amount), candidates are **disambiguated spatially** by proximity to the LLM-estimated box — the LLM placed each row's box at a roughly-correct distinct position — so repeated values don't all collapse onto the first matching line; if there's no usable reference to disambiguate, the field keeps its LLM box rather than risk the wrong one. On a match, the field's `geometry` is swapped (same `[{boundingBox, page}]` shape, **0–1**, 1-indexed page — no UI change needed) and two additive keys are written: `geometry_source` (`"ocr"` | `"ocr-paragraph"` | `"llm"`) and, when the matched OCR line has confidence, `ocr_confidence` (0–1). The LLM `confidence`/`confidence_reason` are **never overwritten**, so HITL thresholds and `confidence_threshold_alerts` are unaffected. Covers both the standard and **granular** assessment paths via a shared `idp_common.assessment.ocr_grounding` module. Gated by a new `assessment.ground_geometry_in_ocr` flag (**default on**, editable in the Configuration UI). The `{OCR_TEXT_CONFIDENCE}` prompt and token budget are **unchanged** (`pageData.json` is read directly from S3, not injected into the prompt). **Safe fallback to today's behavior**: when `pageData.json` is absent (older documents), a backend provides no geometry (plain LLM OCR, Chandra, `none`), or no value match is found, the existing LLM-estimated box is kept — so the worst case is identical to LLM-only behavior. See [docs/assessment-bounding-boxes.md](docs/assessment-bounding-boxes.md) and [lib/idp_common_pkg/idp_common/assessment/README.md](lib/idp_common_pkg/idp_common/assessment/README.md).

- **Per-job `configurationVersion` on the headless Jobs API (#387)** — `POST /jobs` now accepts an optional `configurationVersion` field so callers can pin a specific configuration version per submission instead of always using the stack's active/default config (enables A/B testing and gradual config migrations). The value is validated (≤128 chars, `^[a-zA-Z0-9._-]+$`; invalid values return 422), persisted on the DynamoDB job record, and re-applied by the batch pre-processor as `config-version` S3 metadata on each extracted file — the form the pipeline's `Document.from_s3_event` already reads to select the processing config. `GET /jobs/{id}` now surfaces `configurationVersion` in its response. Fully backward compatible: the field is optional and defaults to `null`, in which case behavior is identical to before (pipeline uses the active config). The version must already exist in the configuration table; an unknown version falls back to pipeline mode downstream. See [docs/govcloud-batch-api.md](docs/govcloud-batch-api.md).

- **Consolidated OCR page data (`pageData.json`) — text + confidence + geometry in one backend-agnostic artifact** — OCR previously scattered a page's data across three files at incompatible granularities: markdown in `result.json`, LINE confidence in the token-reduced `textConfidence.json`, and geometry only inside `rawText.json` (and only for Textract-format backends). `OcrService` now also writes a per-page **`pageData.json`** that consolidates **text + confidence + geometry** into a single **LINE-primary** schema (with optional **WORD** children), where `confidence` and `geometry` are *independently optional* per unit so each backend contributes whatever it has — Textract: per-LINE & per-WORD box + polygon; Mistral LambdaHook: per-LINE confidence with a paragraph-level box shared by sibling lines (`geometrySource: "paragraph"`); Chandra/plain-LLM: text-only lines; `none`/converted: empty or placeholder. Geometry is normalized **0–1** (Textract convention), matching the UI bounding-box renderer. The artifact is derived **in-process** from the same OCR result (no extra OCR calls) and surfaced via a new `Page.ocr_page_data_uri` field (`OcrPageDataUri` in AppSync/DynamoDB/GraphQL). The Web UI **Page viewer** (`PageTextEditorModal`) gains a **"Visual Editor"** view (now the default) modeled on the section-level Visual Editor: the page image on the left and the OCR text lines on the right, where clicking a line draws **only that line's** bounding box on the image (confidence shown per line and color-coded), reusing the existing `PageImageViewer`/`BoundingBox` components. It degrades gracefully (image only, with an info banner) when a backend provides no geometry. The image supports mouse-wheel zoom and click-drag pan, and **Next/Previous page navigation** across the whole document (matching the section Visual Editor). The view is read-only inspection; the existing Text+Markdown / Text+Confidence tabs remain editable only in Edit Mode. Fully **additive and backward-compatible**: existing files and the `{OCR_TEXT_CONFIDENCE}` assessment prompt are unchanged (**zero token-budget impact**), and documents processed before this change simply lack `pageData.json`. Document export includes the new artifact when present. (Assessment grounding on this consolidated geometry is designed but deferred to a later phase.) See [lib/idp_common_pkg/idp_common/ocr/README.md](lib/idp_common_pkg/idp_common/ocr/README.md#consolidated-ocr-page-data-pagedatajson).

- **Agentic extraction: full-schema validation + model escalation (Phase 1)** — Agentic extraction can now validate its output against the **complete class JSON Schema** and, on failure, **escalate to a stronger model**. The generated Pydantic model already enforces `enum`/`pattern`/numeric-bounds/`minItems`, but it does **not** enforce JSON-Schema `format` keywords (`date`, `date-time`, `email`, `uri`, `uuid`, ...); a new `idp_common.extraction.validation` module closes that gap, collects *all* violations at once with readable field paths, and feeds them back to the agent for an extra self-correction round. A new `extraction.agentic.validation` config block controls it: `enabled` (default **false** — no behavior change on upgrade), `check_formats` (default true), `fail_action` (`warn` | `escalate` | `reject`, default `escalate`), and `escalation_model`. When `fail_action: escalate`, only the **failing top-level fields** are re-extracted with a stronger tier model (per-class override via the new `x-aws-idp-extraction-escalation-model` schema extension → global `escalation_model` → the extraction model itself) and merged back into the result — scoping to the failing fields keeps the schema/prompt/output small (cheaper and faster than re-running the whole section) and preserves the already-valid fields; the merged result is kept only if it is valid or has strictly fewer errors (falls back to whole-section re-extraction when failures aren't a field subset). For auditability each section's `metadata` records the `extraction_model` used (and whether it was a per-class override), and `metadata.validation` captures the rules enforced (`check_formats`), `fail_action`, before/after error counts, and escalation details (`escalation_scope`, `escalation_fields`, `escalation_model`, `resolved_by_escalation`). The global `validation` block is editable in the Configuration UI (Extraction → Agentic Extraction → Schema Validation & Escalation), and the per-class escalation-model override is editable in the Document Schema editor. `null` values are treated as **absent** (matching the "return null if not found" prompt convention and the generated model's `Optional[...] = None` fields), so an optional field left null passes while a required field left null is reported as a `required` violation. Note: JSON-Schema `format: date` is ISO-8601 (`YYYY-MM-DD`); the default prompt's `MM/DD/YYYY` will fail format validation, so set `check_formats: false` or use `pattern` for non-ISO dates. See [lib/idp_common_pkg/idp_common/extraction/README.md](lib/idp_common_pkg/idp_common/extraction/README.md) and the demo notebook `notebooks/misc/agentic-extraction-validation-and-escalation.ipynb`.

- **Mistral OCR (serverless) LambdaHook + structured OCR confidence/geometry capture** — New sample Lambda hook `samples/lambda-hook-inference/GENAIIDP-mistral-ocr-hook/` integrates the hosted [Mistral OCR](https://mistral.ai/news/ocr-4/) API (`POST https://api.mistral.ai/v1/ocr`, Bearer-key auth) as an OCR backend via the existing LambdaHook feature — fully serverless, no SageMaker endpoint or GPU. Unlike a text-only OCR hook, it requests structured output (`include_blocks=true`, `confidence_scores_granularity=word`) and translates the Mistral response into **Amazon Textract response format** (one `LINE` block per physical line with a real per-line confidence averaged from per-word scores, plus `WORD` blocks and normalized 0–1 bounding-box geometry), returned under a new top-level `textractBlocks` key. To consume it, `OcrService` now detects `textractBlocks` on any bedrock/LambdaHook OCR response (`_extract_bedrock_ocr_artifacts()`) and persists it as the page's `rawText.json` + a real `textConfidence.json` instead of the "No confidence data available from LLM OCR" placeholder — so OCR confidence flows into Assessment (the `{OCR_TEXT_CONFIDENCE}` prompt) and geometry is available for UI highlighting, exactly like the native Textract backend. Text-only hooks (e.g. Chandra) are unaffected (fully backward compatible). The hook reports `usage.pages` for per-page cost metering, with a matching `GENAIIDP-mistral-ocr-hook` entry added to `config_library/pricing.yaml` ($0.004/page). See [docs/lambda-hook-inference.md](docs/lambda-hook-inference.md) and the [samples README](samples/lambda-hook-inference/README.md).

- **Web UI cost view: pricing match for LambdaHook (and other ARN-keyed) metering** — The Estimated Cost table in the document panel now resolves unit prices using exact-then-partial matching (mirroring the backend's `_get_unit_cost`), so a generic pricing entry (e.g. `GENAIIDP-mistral-ocr-hook`) matches a metering key that embeds the full Lambda ARN (`lambda_hook/arn:aws:lambda:…:function:GENAIIDP-mistral-ocr-hook`). Previously the UI did an exact-match-only lookup and showed `Unit Cost: None` / `N/A` for all LambdaHook usage.

- **Per-class extraction prompt overrides (#377)** — Document classes can now override the global extraction prompts on a per-class basis via two new class-schema extensions: `x-aws-idp-extraction-system-prompt` and `x-aws-idp-extraction-task-prompt`. When present on a class, they replace `extraction.system_prompt` / `extraction.task_prompt` for that class only; when absent, the global prompts are used (fully backward compatible). The override task prompt supports the same placeholders as the global one (`{DOCUMENT_CLASS}`, `{ATTRIBUTE_NAMES_AND_DESCRIPTIONS}`, `{FEW_SHOT_EXAMPLES}`, `{DOCUMENT_TEXT}`, `{DOCUMENT_IMAGE}`, `<<CACHEPOINT>>`). This mirrors the existing per-class `x-aws-idp-extraction-model` override and lets independently-optimized single-class configs (e.g. from separate AutoTune runs) preserve their specialized prompts when merged into one multi-class classification + extraction config — without resorting to a `custom_prompts_lambda_arn`, so the prompts stay in `config.yaml` and remain AutoTune-friendly. Both overrides also compose with a configured custom prompt Lambda (resolved as the Lambda's defaults; the Lambda's returned prompts still win). Editable per-class in the Configuration UI's schema builder. See [docs/extraction.md](docs/extraction.md).
...
- **Agentic extraction: `max_concurrent_batches` now actually shards the input (fixes context-window overflow on long sections)** — Concurrent batch extraction previously handed **every** batch agent the *whole* document and merely instructed it (in text) to "ignore the other pages." So raising `max_concurrent_batches` gave no context relief — every agent still loaded the full OCR text + all page images — while multiplying token cost ~N×. On a long/dense section (e.g. a 17-page distributions statement processed as one section) this overflowed the model context on the first turn, surfacing as the misleading Strands error *"Cannot summarize: insufficient messages for summarization"* (`SummarizingConversationManager` cannot summarize a single oversized first turn). Concurrent extraction now **shards the input**: the section's pages are split into **token-budgeted ranges** (`extraction.agentic.shard_token_budget`, default 40,000 ≈ chars/4) and each shard's prompt contains **only its pages'** OCR text and images. `max_concurrent_batches` becomes an upper bound on both parallelism and shard count (a section is split into as many shards as needed to fit, capped at that value), so long sections are bounded to fit the context window in addition to running in parallel. Splits are page-aligned (table rows live on a single page, so list fields concatenate in order with no row loss); the first page's text is propagated to later shards as header context so column headers / page-1 scalars survive the split; and scalar fields merge by **first-non-null** with disagreements recorded in `metadata.shard_scalar_conflicts`. New `extraction.agentic.shard_token_budget` config (editable in the UI). Single-pass extraction (`max_concurrent_batches: 1`, the default) is unchanged. See [lib/idp_common_pkg/idp_common/extraction/README.md](lib/idp_common_pkg/idp_common/extraction/README.md).

- **Agentic extraction: clear error on context-window overflow** — When an extraction input exceeds the model's context window, the agent loop now raises an actionable message (enable concurrent sharding / lower `shard_token_budget`; enable table parsing — which needs Textract TABLES so OCR emits Markdown tables; reduce attached page images; or use a larger-context `:1m` model) instead of the opaque underlying *"insufficient messages for summarization"* error.

- **Page Viewer: selecting an OCR line jumped back to page 1, and page navigation moved to footer buttons** — In the Web UI Page viewer's Visual Editor (introduced with `pageData.json`), clicking a text line to highlight its bounding box reset the view to the first page on multi-page documents. The selected line's geometry was tagged `page: 1` (hardcoded), and `PageImageViewer`'s geometry-driven page switch then snapped to page 1; it now carries the page being viewed, so highlighting a line stays on the current page. Page navigation is also now exposed as prominent **Previous Page / Next Page** buttons in the modal footer (matching the section Visual Editor's Previous/Next Section), in addition to the existing image arrows.

- **Document Data panel: no confidence shown for fields the assessment decomposed into sub-keys** — When the assessment model split a single plain-string field (e.g. `Insurance Company`, `Named Insured(s) and Mailing Address`) into sub-keyed child assessments instead of returning a flat `{confidence}` on the field, the field object had no top-level `confidence`, so the Document Data panel displayed no confidence at all for it. `getFieldConfidenceInfo` now aggregates the direct children — using the **minimum** child confidence (the worst-case signal for review) and that child's threshold — so these fields show a confidence again. Relatedly, OCR geometry grounding now grounds such decomposed-string fields by matching each sub-key (a fragment of the extracted value) to its OCR line, so they get real bounding boxes on the correct page instead of falling back to the LLM estimate.

- **Agentic extraction silently dropped nested-object field values** — When a class JSON Schema declared nested object (or array-of-object) properties whose names contain spaces (e.g. `"Date of Birth"`, `"Marital Status"`), agentic extraction returned `null` for nearly every nested field while extracting top-level fields correctly — materially less accurate than traditional extraction on the same document. Root cause: `create_pydantic_model_from_json_schema` (`idp_common/schema/pydantic_generator.py`) sanitizes spaced property names into Python identifiers (`Date_of_Birth`) with the original name as a Pydantic alias, but applied the alias round-trip config (`populate_by_name` / `serialize_by_alias` / `validate_by_*`) **only to the top-level model** — nested sub-models kept an empty config, so they validated/serialized by field name and dropped or mis-keyed aliased values. The config is now propagated recursively to every nested model (objects, arrays of objects, and deeper) and the models rebuilt, so nested fields round-trip by their spaced aliases exactly like top-level fields. This aligns nested extraction keys with how Evaluation and Assessment look up values by schema property name (they previously failed to find underscored nested keys). Affects any nested schema with spaced field names; PascalCase/underscore-free schemas were unaffected.

- **Agentic extraction completeness heuristic (silent-loss detection)** — Added an advisory check that, after extraction, computes the fraction of schema-defined leaf fields (recursing into nested objects and array-item schemas) that came back populated, and logs a warning + flags `metadata.population_check` (`fields_defined`, `fields_populated`, `population_ratio`, `below_threshold`, `empty_fields`) when it falls below `extraction.agentic.validation.min_population_ratio` (default `0.5`; set `0` to disable). This surfaces silent extraction loss — such as the nested-field bug above, or a table that extracted zero rows — that schema validation alone cannot catch (sparse-but-valid output is still schema-valid). Advisory only: it never fails extraction (a genuinely sparse document scores low too).

- **`idp-feature-cli deploy-pack` was create-only (#383)** — `deploy_pack()` called `cfn.create_stack(..., OnFailure="DELETE")` unconditionally, so re-running `deploy-pack` against an already-deployed wrapper stack failed with `AlreadyExistsException` — the only way to push pack changes was to delete and recreate the whole stack (rebuilding the IDP host, ~20 min, losing uploaded data) or bypass the CLI with a raw `aws cloudformation update-stack`. This was inconsistent with `idp-feature-cli deploy`, `idp-cli deploy`, and `idp-mp-sim deploy`, which all create-or-update. `deploy_pack()` now delegates to the existing `create_or_update_stack()` helper: it **creates** the wrapper if absent and **updates** it in place if present, treats a no-op update ("No updates are to be performed") as success (exit 0, "already up to date"), refuses to update a CREATE-failed `ROLLBACK_COMPLETE`/`ROLLBACK_FAILED` stack with a clear "delete it first" message, and handles `UPDATE_*` wait states. For a pack whose feature install is a nested `AWS::CloudFormation::Stack`, an in-place wrapper update with a bumped `FeatureVersion` cascades into the nested stack and picks up the republished version-pinned artifacts; the wrapper's host custom resource treats `Update` as a no-op so the running host is undisturbed. See [lib/idp_feature_sdk/README.md](lib/idp_feature_sdk/README.md).
...
- **Extraction config UX: progressive disclosure + surfaced hidden options** — The agentic-extraction configuration in View/Edit Config was confusing: four levels of nesting with every sub-knob shown unconditionally (table-parsing thresholds even when table parsing was off, validation/escalation knobs even when validation was off, the whole agentic block even when agentic was off). The config-form schema (`patterns/unified/template.yaml`, `extraction:` block) now uses the existing `dependsOn` mechanism to **progressively disclose** options: `agentic.*` sub-options (table_parsing, validation, max_concurrent_batches) appear only when Agentic Extraction is enabled; table-parsing thresholds appear only when the parse_table tool is enabled; validation sub-fields appear only when validation is enabled; and the escalation model appears only when `fail_action: escalate`. Labels, ordering, and help text were clarified (each option states when it applies and the relevant cost/latency tradeoff). Three **load-bearing options that were previously editable only via YAML** are now surfaced in the UI: `table_parsing.max_empty_line_gap`, `table_parsing.auto_merge_adjacent_tables`, and the `missing_field_handling` block (BLANK vs MISSING fields). This is a **presentation-only** change — no config keys were renamed or removed, all stored configs and evaluation baselines load unchanged, and the backend reads the same fields as before. (`extraction.image.dpi`/`preprocessing` were intentionally **not** surfaced here: they are read by OCR config, not extraction, so they would be no-ops under Extraction.) See [lib/idp_common_pkg/idp_common/extraction/README.md](lib/idp_common_pkg/idp_common/extraction/README.md).
