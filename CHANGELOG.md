...
- **Optional `{CLASS_AND_ATTRIBUTE_NAMES_AND_DESCRIPTIONS}` placeholder for classification prompts** ([#262](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/262)) — Pattern 2 classification `task_prompt` templates can now reference an opt-in `{CLASS_AND_ATTRIBUTE_NAMES_AND_DESCRIPTIONS}` placeholder that expands, for each document type, to the class name, description, **and** the schema attribute (extraction-field) names declared on the class. Two render variants ship: an XML-tagged listing for `multimodalPageLevelClassification` prompts and a three-column markdown table (`type | description | attributes`) for `textbasedHolisticClassification` prompts. The placeholder is fully **cost-neutral by default** — the existing `{CLASS_NAMES_AND_DESCRIPTIONS}` placeholder and all stock prompts in `config_library/` are unchanged, and the new substitution key is only materialized into the prompt when the template references it. Useful for schema-rich domains (lending, healthcare, insurance) where two classes share similar names/descriptions but have very different extraction schemas (e.g. `appraisal_report` vs `inspection_report`). The schema walker flattens nested objects to dotted paths (`borrower.address.zip`) and unwraps arrays of objects (`comparable_sales.address`); per-class attribute counts are soft-capped at `ClassificationService.MAX_ATTRIBUTES_PER_CLASS` (default 50) with a `...(+N more)` truncation marker and a warning log to keep prompt cost predictable. Classes with no JSON Schema render `<attributes>(no schema)</attributes>` so the absence is obvious for debugging. The Web UI **Configuration → Prompt Preview** tab knows about the new placeholder: it renders the live attribute listing in green when a custom prompt references the placeholder, and shows it as a separate "Substitution Details" panel on the Classification step so users can see exactly what gets injected. The CloudFormation `ConfigSchema` description for the classification `task_prompt` field also documents the new placeholder so users editing prompts in the Web UI Configuration Editor can discover it. See [`docs/classification.md`](docs/classification.md#optional-class_and_attribute_names_and_descriptions-placeholder) for the schema-walking rules, rendered output examples, and recommended use cases.

- **Cross-account Bedrock invocation via STS AssumeRole** ([#305](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/305)) — IDP processing Lambdas can now route all Amazon Bedrock data-plane and control-plane traffic through a centralized "hub" AWS account. Set the new optional CloudFormation parameter `BedrockHubRoleArn` (with optional `BedrockHubRoleExternalId` and `BedrockHubRoleSessionName`) to the ARN of an IAM role in a hub account; the stack conditionally adds `sts:AssumeRole` to each affected Lambda's execution role and injects the role configuration as environment variables. A new `idp_common.bedrock.session.get_bedrock_session()` factory wraps `botocore.credentials.DeferredRefreshableCredentials` so STS credentials are auto-refreshed before expiry — warm Lambdas don't break after the default 1-hour STS session. **Coverage**: the entire pipeline (classification, extraction traditional + agentic, assessment, summarization, evaluation, OCR, rule-validation), discovery (classes, rules, multi-doc Embed/Cluster/Analyze/Save), embeddings, CachePoint inference-profile resolution, **Chat with Document** (`chat_with_document_processor`), and **Agent Companion** (`agent_chat_processor`, `agent_processor`, AgentCore MCP handler — including all Strands sub-agents). The change is fully additive: existing same-account `bedrock:*` IAM statements are kept in place so leaving the parameter empty preserves prior behavior. **Out of scope for v1**: BDA (Pattern-1) runtime calls, Bedrock Knowledge Bases (`bedrock-agent-runtime` is a separate identity surface), and `idp_common/model_finetuning/` utilities. See [`docs/cross-account-bedrock.md`](docs/cross-account-bedrock.md) for the architecture, sample hub-role policies, resource-scoping caveats (application inference profiles and Guardrail IDs must be in the hub account), the STS VPC endpoint requirement for private-network deployments, and a step-by-step verification checklist.

- **Distinguish MISSING pages from BLANK fields in extraction output** ([#317](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/317)) — for sparsely-populated multi-section forms (worksheets, supplements, etc.) where entire pages may be legitimately omitted, the extraction service can now distinguish between fields whose source page was *present but empty* (BLANK) and fields whose source page was *not submitted* (MISSING). Two new optional schema extensions are introduced: `x-aws-idp-page-types` declares the named page sub-types a class can include (each with a content-detecting regex), and `x-aws-idp-source-page-types` declares which page types each property's source data lives on. When a section is processed, a deterministic regex-based resolver detects which declared page types are present, surfaces the result to the LLM via annotated `--- PAGE N [PageType] ---` markers in the prompt, and (when `extraction.missing_field_handling.enabled: true`) post-processes the extracted JSON to drop or null-out fields whose declared source pages are absent. Output JSON gains optional `page_type_resolution` and `missing_fields_report` siblings to `inference_result`. Fully additive and backwards-compatible — no behavior change unless the new extensions are present in the schema and the config knob is enabled. The Document Schema editor in the Web UI gains first-class form widgets for both extensions: a new "Page Types" editor on the class inspector and a "Source Page Types" multi-select on each property's inspector (driven by the parent class's declared page-type names). See [`docs/missing-page-handling.md`](docs/missing-page-handling.md) for the feature guide and [`notebooks/usecase-specific-examples/multi-page-bank-statement/step3_extraction_with_missing_pages.ipynb`](notebooks/usecase-specific-examples/multi-page-bank-statement/step3_extraction_with_missing_pages.ipynb) for a runnable end-to-end demo.

- **Private (VPC-only) deployment — browser uploads route through the S3 Interface VPC Endpoint** — when `WebUIHosting=ALB` is selected, the ALB nested stack provisions an S3 Interface VPC Endpoint and exposes its full regional DNS name (with the AWS-assigned random suffix preserved) as the new stack output `S3VPCEndpointDnsName`. The Web UI presigner Lambdas (`UploadResolverFunction`, `DiscoveryUploadResolverFunction`, `TestSetResolverFunction`, `TestSetZipExtractorFunction`), `ApiHandlerFunction`, and the `ConfigurationCopyFunction` and `FccDatasetDeployerFunction` custom resources receive a new `S3_ENDPOINT_URL` env var pointing at `https://bucket.<vpce-id>-<random>.s3.<region>.vpce.amazonaws.com` and use boto3 with `addressing_style="virtual"` so the SigV4 host header matches the VPCE bucket-vhost DNS. Browser uploads, document discovery uploads, and Lambda S3 traffic stay on the AWS backbone with zero public-internet egress.

- **Bring-Your-Own S3 VPC endpoint** — two new top-level CloudFormation parameters allow customers with a central network account to reuse an existing endpoint instead of having the IDP stack provision one:
  - `S3VpcEndpointIdOverride` — existing S3 Interface VPC Endpoint id (`vpce-...`)
  - `S3VpcEndpointDnsNameOverride` — the endpoint's regional DNS name (with AWS-assigned random suffix, leading `*.` stripped)

  Both parameters must be set together; CloudFormation `Rules` enforce the pairing and fail the change-set creation with an actionable message if only one is supplied.

- **`monitoring` (CloudWatch) Interface VPC Endpoint** — `scripts/vpc-endpoints.yaml` now provisions a CloudWatch Interface VPC Endpoint, gated by the new `CreateMonitoringEndpoint` parameter (default `true`). Without it, the `DashboardMerger` custom resource (which calls `cloudwatch:GetDashboard` / `PutDashboard` during stack create / update) hangs in private mode. `scripts/check-vpc-endpoints.sh` is updated to detect and skip this endpoint when it already exists.

- **`LambdaSecurityGroupId` parameter on the ALB nested stack** — when supplied, the ALB-stack S3 VPC Endpoint security group accepts inbound 443 from the app Lambda security group, so VPC-resident Lambdas can reach S3 through the same VPC endpoint that ALB uses. Previously app Lambdas were blocked by an SG mismatch and stack create hung in `ConfigurationCopyFunction` for 5 minutes before timing out.


### Changed

- **ALB nested stack S3 VPC endpoint policy scoped to same-account operations** — the endpoint policy allows a finite set of S3 actions (`GetObject`, `PutObject`, `ListBucket`, `DeleteObject`, `AbortMultipartUpload`, `GetBucketLocation`, `ListMultipartUploadParts`) with `Resource: "arn:${AWS::Partition}:s3:::*"` conditioned on `aws:PrincipalAccount` and `aws:ResourceAccount` matching the deployment account. The wildcard resource is necessary because the policy must cover every bucket the VPC touches (all created in the parent stack, creating cyclic dependencies if enumerated). Authorization is additionally enforced at the network layer (endpoint security group restricts traffic to the ALB and Lambda SGs) and at the IAM layer (Lambda execution roles + bucket policies). See `docs/deployment-private-network.md` for the security-trade-off discussion.
...
- **Bedrock Knowledge Base nested stack no longer left in `DELETE_FAILED` on update/delete** ([#315](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/315)) — two related reliability fixes in `nested/bedrockkb/template.yaml`:
  - **Reliable `AWS::Bedrock::DataSource` deletion during sync** — the `S3DataSource` is on a 30-minute ingestion schedule, and `DeleteDataSource` returns `ConflictException` if a job is `IN_PROGRESS`. The `StartIngestionJobFunction` custom resource Delete handler (`nested/bedrockkb/src/start_ingestion_job_custom_resource/handler.py`) now lists in-progress / starting / stopping ingestion jobs, calls `StopIngestionJob` on each, and polls `GetIngestionJob` until terminal status (or a 12-minute deadline) before signalling SUCCESS — so CFN can then delete the data source cleanly. The handler always reports SUCCESS on Delete (logs warnings instead) so a stuck job never blocks the stack delete. The function's IAM role now has `bedrock:StopIngestionJob`, `bedrock:GetIngestionJob`, `bedrock:ListIngestionJobs`, the timeout is bumped to 15 min, and `StartS3IngestionJob` / `StartWebIngestionJob` `DependsOn` their respective `*DataSourceScheduler` so CFN tears down the schedule first (eliminating the race where the cron fires a new job mid-drain).
  - **Helper IAM roles now `DeletionPolicy: Retain` / `UpdateReplacePolicy: Retain`** — `DataSourceSchedulerRole` and `StartIngestionJobFunctionRole` are pure ephemeral helpers (only assumed by EventBridge Scheduler and the helper Lambda, respectively, both of which are deleted with the stack). Marking them `Retain` decouples nested-stack delete success from the deploying principal's `iam:DeleteRole` permission. This is a defensive fix for environments where a session policy denies `iam:DeleteRole` — previously this left `*-DOCUMENTKB-*` in `DELETE_FAILED` while the parent stack reported `UPDATE_COMPLETE`, requiring manual cleanup. The two retained roles are inert; they can be deleted manually after the stack is gone if desired.
- 
