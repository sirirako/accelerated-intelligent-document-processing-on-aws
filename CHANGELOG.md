...
- **Bedrock circuit breaker** — a CFN-parameterized circuit breaker that pauses new workflow starts when Bedrock is unhealthy and auto-recovers once the service comes back, so transient Bedrock outages no longer burn through SQS retries or leave documents half-processed. Off by default for full backward compatibility.
  ...
  - **New CFN parameters** (all default to off, fully backward-compatible): `EnableCircuitBreaker`, `CircuitBreakerRecoveryTimeoutSeconds`, `CircuitBreakerErrorHandlerArn`. New env var `CIRCUIT_BREAKER_ENABLED` on `queue_processor` and `workflow_tracker` (the `CIRCUIT_BREAKER_ID = "circuit_breaker"` DDB partition key is a shared Python constant, not an env var).
  ...
  - **Round-2 fixes:**
    - `opened_at` is now cleared on every `CLOSED` transition (manager `manual_close`/`reset` and `workflow_tracker` auto-close) so the details panel stops showing a stale outage timestamp after recovery.
    - `CircuitBreakerManagerFunction` and `CircuitBreakerResolverFunction` now attach to the VPC when `AppSyncVisibility=PRIVATE` (resolver + manager) or `DeployInVPC=true` (manager only). Previously the manager's SigV4 `urlopen` to a private AppSync endpoint and the resolver's DynamoDB reads would fail from the AWS-managed network in VPC-only accounts.
    - `manual_open` no longer increments `failure_count` — the operator-visible counter tracks Bedrock failures, not admin pauses.
    - Resolver `is_iam` gate now also requires `accountId` on the identity object (belt-and-braces for the `publishCircuitBreakerStatus` IAM-only check).
    - Badge's OPEN-state error tooltip migrated from a `<span title>` wrapper to a Cloudscape `Popover` for consistency with the rest of the UI.
    - Docs: transition diagram now correctly shows alarm-OK on OPEN → HALF_OPEN (not CLOSED); Resume-processing bullet notes that the action is unconditional and an active outage will re-open the breaker.
...
- **SRT now uses `--no-license-update` flag to prevent automatic license header modifications during security scans** — Previously SRT could automatically modify source file license headers during scans, which was undesirable for version control. The flag is now passed to prevent unwanted file modifications.

- **Agentic extraction with Claude Opus 4.7 no longer fails with `top_p is deprecated` ([GitHub #304](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/304))** — The Claude 4.7+ enablement in v0.5.7 correctly taught the traditional Bedrock invocation path (`idp_common/bedrock/client.py`) to omit the deprecated `temperature` / `top_p` / `top_k` parameters, but the Strands-based **agentic** extraction path (`idp_common/extraction/agentic_idp.py`) was missed and still forwarded the config's defaulted `top_p=0.1` to ConverseStream, causing a `ValidationException: \`top_p\` is deprecated for this model.` at the extraction stage. The agentic path now shares the same Claude 4.7+ model detection (`is_claude_4_7_model`) as the traditional path and returns an empty inference-params dict for those models, so neither path can ever send a deprecated parameter. Also promoted `_is_claude_4_7_model` → `is_claude_4_7_model` as a public helper (with a back-compat alias) and added `tests/unit/extraction/agentic_idp/test_inference_params.py` regression tests covering all us./eu./global. region prefixes and the `:1m` variant.
