...
- **API RBAC test system — static scan + live authorization harness (`make api-test`).** A two-layer, auditable test system that verifies every UI API operation (`POST /op/{field}`) is protected by its required Cognito group(s) and, where applicable, config-version scope. Because the REST API Gateway authorizer only *authenticates* (all group/scope checks live in the resolver Lambdas), a missing server-side check ships an operation that looks protected in the schema but isn't — so authorization testing is a primary control. A single source of truth (`scripts/api_rbac_expectations.yaml`, 96 operations + a tracked known-gaps register) feeds both layers so they can't drift. **`make api-test-static`** (`scripts/sdlc/scan_api_rbac.py`) runs with no AWS and is CI-safe: it cross-checks the operation universe (dispatcher `FIELD_FUNCTION_MAP` ∪ `ddb_direct` ∪ aliases) against the schema directives and the expectations file, flagging missing checks and silently-ignored `@aws_auth` directives. **`make api-test STACK_NAME=<stack>`** (`scripts/test_api_rbac.py`) drives a deployed stack: it creates temporary Cognito users (one per group + a config-version-scoped Author), calls every operation as every role plus unauthenticated and with malformed/tampered tokens, seeds a scoped user and asserts out-of-scope config versions are denied, and writes a JSON/Markdown report (with git SHA, account, and request IDs) before tearing the users down. Known gaps surface as WARN so real regressions fail the gate. Documented as a skill (`.claude/skills/api-rbac-test.md`) and reflected in the RBAC threat model (new AUTH.T07 config-version scope bypass, AUTH.T08 silently-ignored schema directives). This harness found the config-version scope fail-open fixed in this release.

- **In-app feedback & GitHub issue reporting.** Users can report bugs or request features on the project's public GitHub repository directly from the web UI, with deployment details pre-filled. Entry points: a **Feedback** menu in the top navigation (Report a bug / Request a feature / View existing issues); a **Report this issue on GitHub** button in the [Error Analyzer](docs/error-analyzer.md) Troubleshoot modal footer (pre-fills the bug form with document context — object key, status, config version, execution ARN, job error — plus the agent's Markdown findings, alongside a **Copy full details** clipboard button for content the length-capped URL can't carry); a **Create GitHub issue** button under the latest answer in the [Agent Companion Chat](docs/agent-companion-chat.md) (attaches the agent's answer as findings); and a **Report an issue** link in the side-nav Resources section plus a **Feedback & support** block in the Document List help panel. The in-app links pre-fill the issue **body** directly (via `?title=&body=&labels=`) with an environment summary (Version, Build, Stack name, Region, Processing Mode) sourced from the deployment settings, plus context (document + agent findings for the Troubleshoot path; the agent answer for the chat path) — so the content is embedded immediately. This intentionally bypasses the `.yml` issue *forms* in `.github/ISSUE_TEMPLATE/` (converted from the prior Markdown templates and still applied when a user clicks **New issue** directly on GitHub), because GitHub ignores a pre-filled `body` when a form template is selected. Nothing is submitted automatically — GitHub always shows the pre-filled form to review first, and it carries a prominent reminder to redact sensitive document data (issues on the public repo are visible to everyone). The **Troubleshoot modal** was also made non-dismissing (closes only via its Close button — an outside click or Esc no longer tears down an in-progress analysis) and gained a **Minimize** button that collapses it to a restore chip while the analysis continues. See [web-ui.md](docs/web-ui.md#feedback--issue-reporting).

- **View/Edit Configuration UX improvements.** The configuration editor gets a cluster of usability fixes: the crowded toolbar is de-cluttered (secondary actions — Export, Save as default, Restore default (All), Save as Version, BDA sync — collapse into a single **Actions** menu, leaving the view toggle, Refresh, and primary Save changes prominent), disabled Save/action buttons now carry tooltips explaining why (e.g. stack-managed versions), and the unsaved-changes banner + **Discard** now appear on all versions (including the read-only `default`). The SPA navigation guard restores the exact previous URL (preserving `?version=`/`?tab=` deep links) instead of bouncing to the base config route. Prompt fields render in taller textareas with an **Expand editor** modal for full-height editing. The Configuration Versions table is expanded by default and uses native Cloudscape multi-selection (replacing raw checkboxes); version delete now requires a **confirmation modal**. Toggling **Use BDA** shows an inline warning that it swaps the whole pipeline. Version comparison shows full untruncated values and, for two-version compares, a word-level inline diff (added=green, removed=red). Save/version/reset actions surface a **success toast** in the sticky notification area (visible even when scrolled deep). Managed/Custom/Active badges gained hover tooltips describing each. The **Document Schema** builder (Schema Builder) also improves: **Export** is now a menu offering **Export all** or **Export "&lt;class&gt;"** (the selected document type plus only the shared classes it references); the **Add Attribute** dialog gains an **Add another** button that saves and immediately starts a new attribute (so multiple fields can be added without reopening); a newly-created class with no attributes shows an **Add first attribute** button instead of only a hint; and **Show Preview** now leads with a graphical **Diagram** tab — a dependency-free entity-relationship view of the classes (nodes list attributes; `$ref`/array-of-`$ref` relationships are drawn as labelled edges, solid for a single object and dashed for a list; click a class to open it) alongside the existing JSON Schema and Statistics tabs. See [web-ui.md](docs/web-ui.md) and [configuration-versions.md](docs/configuration-versions.md).

- **API Gateway access logging at `LogLevel` INFO/DEBUG.** When the `LogLevel` stack parameter is `INFO` or `DEBUG`, access logging is enabled on the web UI's REST API stage. Access logs record request metadata only (JSON: request id, source IP, method/path, status, latency, integration status, authorizer error/status, WAF response, user agent) — never request/response bodies — and capture requests that fail **before** reaching the dispatcher Lambda: Cognito authorizer 401/403s, WAF blocks, CORS/gateway responses, and the S3-proxy Web UI routes (which have no Lambda at all). Logs go to a stack-managed, KMS-encrypted log group honoring `LogRetentionDays`. REST API access logging requires the account-level API Gateway CloudWatch role, so the stack also registers one (`AWS::ApiGateway::Account` + a role with the managed `AmazonAPIGatewayPushToCloudWatchLogs` policy); this account/region-wide setting is retained on stack deletion so other stacks' API logging is unaffected. At `WARN`/`ERROR` (recommended for production) no access logging is configured, matching the prior behavior.

- **API Gateway execution logging at `LogLevel` INFO/DEBUG (ERROR level, no payloads).** Alongside access logging, the REST API stage now enables CloudWatch **execution** logs via `MethodSettings` — deliberately at `LoggingLevel: ERROR` with `DataTraceEnabled: false`, because INFO-level execution logging echoes full request/response payloads (customer document data) into CloudWatch. ERROR captures the gateway-side failures access logs can't diagnose (integration mapping errors, MOCK/CORS template failures); `MetricsEnabled: true` adds per-method CloudWatch metrics. The auto-created `API-Gateway-Execution-Logs_<api-id>/api` log group is pre-declared by the stack so it gets KMS encryption and `LogRetentionDays` retention instead of API Gateway's default unencrypted/never-expire group. Resolves SRT finding API-GW-006 by mitigation rather than suppression.

- **Feature SDK: shell-free structured build/package commands in `feature.yaml`.** The feature manifest gains structured step-list forms `ui.build` and `agentSource.package` — each step an `{argv: [...], cwd: ...}` object executed with `shell=False` (direct argv exec: no `&&`, pipes, globs, or variable expansion; `cwd` replaces the `cd X && ...` idiom), eliminating the shell command-injection surface (Bandit B602) and keeping manifests portable across shells. Step `cwd` directories are validated at manifest load, and publishing aborts on the first non-zero exit. The legacy single-string `ui.buildCommand` / `agentSource.packageCommand` forms remain supported for existing manifests but are deprecated (they run through a shell and now emit a runtime deprecation notice); a manifest may declare either form, not both. All in-repo feature manifests (feature-template, sample-feature, sample-health-insurance-review) are migrated to the structured form. See [lib/idp_feature_sdk/README.md](lib/idp_feature_sdk/README.md#build--package-commands-in-featureyaml).

- **Discovery: create a new configuration version inline, and a "Replace" save mode.** The Discovery panels (Single Document, Multiple Documents, and Policy Discovery) gain two options next to the Configuration Version selector. **Create new version** opens a modal that creates a fresh configuration version — inheriting its settings and existing document classes from a chosen source version — without leaving Discovery, then selects it as the save target. A **Save mode** control chooses how discovered schema is persisted: **Add to existing schema** (default — the prior behavior, merging discovered classes into the version and deduplicating by `$id`) or **Replace existing schema**, which removes all existing document classes in the selected version before saving only the newly discovered ones (a warning is shown while Replace is selected). Replace is implemented by clearing the target version's schema list **once, up front, in the discovery upload resolver** (`_clear_version_schema`) before any jobs are enqueued — so multi-section and multi-document runs (which spawn N per-class jobs that each merge into the same version) correctly rebuild a clean schema rather than leaving only the last class. Class discovery clears `classes`; Policy Discovery clears `policy_classes`; all other config sections are preserved. Threaded through the `uploadDiscoveryDocument` and `startMultiDocDiscovery` mutations via a new optional `saveMode` argument. See the [Discovery guide](docs/discovery.md#web-ui-interface) and the [discovery README](lib/idp_common_pkg/idp_common/discovery/README.md#saving-discovered-schema-augment-vs-replace).

- **`--govcloud` publish/deploy option — full Web UI in GovCloud (CloudFront removed).** `idp-cli publish` and `idp-cli deploy` gain a `--govcloud` flag that transforms the CloudFormation template to remove every `AWS::CloudFront::*` resource (CloudFront does not exist in GovCloud, so the standard template fails validation there with `E3006 Resource type 'AWS::CloudFront::Distribution' does not exist in 'us-gov-west-1'`) and forces `WebUIHosting=APIGateway`, serving the Web UI as an S3 proxy on the REST API. Unlike `--headless` (which removes the UI entirely), `--govcloud` **keeps the full UI** — it is the way to run the interactive Web UI in GovCloud. The two flags are mutually exclusive. Implemented as a new `GovCloudTemplateTransformer` that collapses every `Fn::If[UseCloudFrontHosting]` to its API-Gateway (else) branch, deletes the CloudFront resources/condition/params and the dead `cloudfront.*` log-delivery bucket-policy statement, removes the `AWS::Lambda::Url` chat *streaming* endpoint + its permission (Lambda Function URLs are unavailable in GovCloud), and fixes ARN partitions. **Chat still works in GovCloud without live streaming**: when no Function URL is configured, the Web UI auto-switches to a non-streaming path — it sends the message over the REST API (which async-invokes the same chat processor) and polls the chat-messages table for the final answer (spinner, then the full answer appears at once; the document-chat processor now persists its final message so it is pollable too). Commercial deployments keep full token streaming; the two share one codebase via `VITE_STREAM_URL`-empty auto-detection. After the transform the tooling automatically runs a **region-aware `cfn-lint`** on `idp-govcloud.yaml` (against the target GovCloud region, or `us-gov-west-1` when building from a commercial region) and **fails the publish/deploy on any remaining `E3006`** GovCloud-unsupported-resource error — so a future template change that reintroduces such a resource is caught at build time, not deploy time. (`cfn-lint` is offline/no-credentials; skipped with a warning if not installed. Verified: the transformed template yields 0 errors for both `us-gov-west-1` and `us-gov-east-1`.) See the [GovCloud Deployment guide](docs/govcloud-deployment.md#keeping-the-web-ui-in-govcloud---govcloud). Requires the API Gateway hosting option (below).

- **API Gateway Web UI hosting (`WebUIHosting=APIGateway`) — private/VPC-capable UI without CloudFront.** A new Web UI hosting option serves the React SPA as an **S3 proxy on the existing api-resolvers REST API**, on the same stage (`/api`) as the data operations. Because the UI and the API share one origin and one stage, the UI automatically inherits the REST API's network posture: set `ApiGatewayVisibility=PRIVATE` (with `DeployInVPC=true` + `VpcId`/`PrivateSubnetIds`/`LambdaSubnetIds`/`LambdaSecurityGroupId`/`ApiGatewayVpcEndpointId`) to make the **UI VPC-only**, and the optional stage-level WAFv2 IP allow-list (`WAFAllowedIPv4Ranges`) covers the UI routes with no extra configuration. Unlike the removed ALB option this needs **no ALB, no ACM certificate, and no S3 VPC Interface Endpoint** — API Gateway reaches the Web UI bucket over AWS-internal networking via an IAM role, and `BinaryMediaTypes: */*` lets it return asset bytes intact (the JSON `/op` data path is unaffected — the dispatcher base64-decodes request bodies). The SPA (HashRouter) is built with Vite `base=/api/` in this mode so assets resolve under the stage prefix; client-side routes live in the URL fragment (`/api/#/...`) so no server-side rewrite is needed (a genuinely missing asset path returns 404). OAuth callback/logout URLs for the API Gateway origin are registered on the Cognito client by a post-deploy custom resource (avoiding a circular dependency). The application URL (`ApplicationWebURL` output) becomes the execute-api `/api/` URL. A CI deployment test exercises this option with **VPC + PRIVATE** on a self-contained test VPC. See the [API Gateway Hosting guide](docs/apigateway-hosting.md).

- **Lower-cost agentic extraction on table-heavy documents (up to ~57% cheaper, no accuracy change).** Two complementary optimizations to the advanced/agentic extraction loop. (1) The deterministic table tools (`parse_table`, `map_table_to_schema`) now return only a **compact model-visible summary** (columns, counts, head/tail sample) as the tool result — the full parsed/mapped rows are kept in agent state and read directly by the downstream mapping/finalize tools, so the dense rows are no longer re-transmitted to the model on every turn of the multi-step agent loop (previously the single largest full-price, after-cache-point cost term). (2) New **`extraction.agentic.table_parsing.lazy_images`** knob (default `true`): when a pre-flight table parse succeeds, page images are **not** pre-loaded into the extraction prompt — the table tool is text/markdown-driven and never reads images, and the agent can still fetch a page on demand via the `view_image` tool. This removes per-turn image re-transmission (which dominates on multi-page documents and can push large documents toward context-window limits). Set `lazy_images: false` for image-dependent corpora where the model must see page layout even when a table is present. A controlled A/B (bank-statement, advanced mode) measured ~57% lower extraction cost with identical list completeness and field accuracy. See the [Extraction & Confidence guide](docs/extraction-and-confidence.md).

- **`DATE` evaluation method for format-insensitive date comparison (Stickler v0.5.0).** Upgraded the evaluation engine from `stickler-eval` 0.4.0 to 0.5.0 and exposed its new `DateComparator` as a first-class evaluation method (`x-aws-idp-evaluation-method: DATE`). It parses both the expected and extracted values into dates before comparing, so surface-form differences no longer count as misses — `01/05/2024`, `2024-01-05`, and `January 5, 2024` all match — and it understands date ranges (e.g. `2024-01-01 to 2024-01-31`). Optional per-field tuning is passed straight through to the comparator via `x-aws-idp-evaluation-method-config` (`dayfirst`, `tolerance`, `range_mode`, `precision_mode`, `allow_partial_year`). The method is available in the Test Studio schema editor method dropdown, and all `format: date` fields across the bundled `config_library` presets (bank-statement, lending-package, healthcare, ds11-passport, ocr-benchmark, realkie-fcc, rule-validation, rvl-cdip) have been switched from `EXACT`/`FUZZY`/`LEVENSHTEIN` to `DATE`. Time-only fields (e.g. `19:02`) and free-text fields that merely mention a date are intentionally left on their prior methods, since `DateComparator` scores time-only values as `0.0`. See the [evaluation guide](docs/evaluation.md) and the [evaluation README](lib/idp_common_pkg/idp_common/evaluation/README.md).

- **Bundled sample documents are now deployable and launchable from the web UI, with one-click matching configuration.** The curated sample documents in `samples/` are published to the deployment's ConfigurationBucket at deploy time (under `samples/`, indexed by `config_library/samples-manifest.json`, which now records each sample's associated config preset via a `configId` field). The **Upload Documents** panel gains a **Document source** toggle: alongside the existing local-file upload, users can pick a bundled sample and process it without downloading it first (a new `uploadSampleDocument` mutation copies it server-side into the InputBucket). When a selected sample is tuned for a Configuration Library preset that isn't imported yet, a pre-checked **"Also import and use its configuration"** option imports that preset as a new configuration version and processes the sample with it; if the config is already imported, that version is preselected. New GraphQL operations: `listSampleDocuments` (Admin/Author/Viewer) and `uploadSampleDocument` (Admin/Author). See [web-ui.md](docs/web-ui.md#upload-documents).

- **BDA as a pure OCR engine (new `ocr.backend: bda`).** Amazon Bedrock Data Automation "standard output" can now be used as an OCR-only engine feeding the normal classification/extraction/assessment pipeline — distinct from the whole-pipeline BDA mode (`use_bda`). It returns reading-order **markdown with tables and layout** plus word-level confidence and bounding boxes in a single call at a flat **$10/1,000 pages**, and auto-enables the agentic extraction table tool. Pages are processed one at a time via the synchronous `InvokeDataAutomation` API (sync BDA is capped at ~10 pages/call, so per-page invocation also yields per-page concurrency and retry isolation); a standard-output-only SYNC project (`GENAIIDP-OCR-StandardOutput`, or `ocr.bda_project_arn`) is auto-created with `modalityRouting` forcing page images to DOCUMENT, and each page is passed by S3 URI so routing reliably applies. BDA output is adapted to Amazon Textract response format (line confidence reconstructed from word confidence, since BDA's own line confidence is unreliable), so it flows through the existing `textConfidence.json`/`pageData.json` path. Choose BDA for table-heavy documents and predictable pricing without composing Textract features; plain Textract remains cheaper for raw text. See the [OCR service README](lib/idp_common_pkg/idp_common/ocr/README.md).

- **Document versions: every processing run of a document is retained, comparable, and downloadable.** Re-uploading or reprocessing a document (same S3 object key) no longer discards the prior results — each successful run is recorded as an immutable version. On completion the WorkflowTracker snapshots the current S3 `VersionId` of every output object into a per-run manifest (`<key>/runs/<run-id>/manifest.json`) and writes a run record (`PK=doc#<key>`, `SK=run#<run-id>`), so a version's exact bytes remain retrievable even after later runs overwrite the outputs (the reserved `runs/` prefix survives reprocessing). The document detail page gains a **Version History** panel to list, **view** any past version's outputs read-only (sections, page text/images, extraction results, and reports fetched from that run's pinned S3 object versions), section-by-section **compare**, and (Admin-only) delete versions; the CLI adds `idp-cli list-versions` and `download-results --document-id … --run-id …`; and the API adds `listDocumentVersions` / `getDocumentVersion` / `compareDocumentVersions` / `deleteDocumentVersion`. Retained for `DataRetentionInDays`; enabled by default (no GSI schema change — run items stay out of `TypeDateIndex`). Bucket lifecycle rules now also expire **noncurrent** S3 object versions (previously they accumulated forever). See the [Document Versions guide](docs/document-versions.md).

- **Model-aware auto-sizing of shards and confidence batches (one `context_buffer` knob).** Shard token budgets and confidence list-batch sizes are now DERIVED from each model's context/output window (`model_config_limits.yaml` gained `max_input_tokens` per family, incl. distinct 1M `:1m` entries) minus a single `extraction.context_buffer` (default 30%) — so you no longer hand-set `shard_token_budget`/`list_batch_size` per model (they remain optional overrides; `0` = auto). Advanced mode now defaults to the resumable **Step Functions sharded path** (`runtime: step_functions`, `max_concurrent_batches: 10`) so large documents shard across per-shard Lambdas (each under the 900s ceiling) instead of a single-agent timeout. Conversation summarization for the extraction agent is also model-window-aware (preserves more recent turns on roomy models). Every sizing calculation is logged (CloudWatch) and shown in the processing report.

- **Concurrent confidence assessment batches (cache-warm then fan-out).** Large-list confidence batches — which are independent — now run concurrently after one cache-warming call (avoiding a Bedrock prompt-cache write storm), collapsing many sequential assessment inferences into a parallel gather while preserving exact row order. Sequential when `max_concurrent_batches <= 1`. Batch count + concurrency are recorded and reported.

- **Processing report shows how the document was processed.** The section Processing Report (and the metadata behind it) now surfaces the applied model window, context buffer, auto-derived shard/batch sizes, batch counts + concurrency, and a compact process-flow visual (OCR → Classify → Extract → Confidence, with fan-out stages highlighted) — plus honest issue reporting: an extraction that under-produced (e.g. an empty large table on Simple mode) is flagged `extraction_incomplete` (with a recommendation to use Advanced mode) instead of a misleading SUCCESS, and a truncated confidence call that recovered nothing no longer falsely claims "recovered".

- **1S-TopK single-stage extraction + confidence for Simple mode.** When Extraction mode is `simple` and Confidence mode is `integrated`, extraction and per-field confidence are now produced in a **single LLM call** (no separate Assessment pass), cutting extraction-workflow LLM invocations in half. The extraction prompt asks the model for its top-K guesses per field with probabilities (`G1/P1` … `GK/PK`); a new `topk_resolver` takes the top guess (`G1`) as the value and its probability (`P1`) as the confidence, then routes the confidence through the shared threshold-enrichment + grounding path so the output `result.json` has the same `inference_result` + `explainability_info` structure as separate mode (downstream evaluation, reporting, UI, and HITL are unchanged) and the standalone Assessment step auto-skips. Requesting ranked alternatives yields better-calibrated, less-overconfident scores than single-value self-assessment (Tian et al., "Just Ask for Calibration", EMNLP 2023); the full candidate set is preserved in `metadata.topk_candidates` for audit. The prompt is editable in the UI ("Task prompt (1-Stage TopK extraction + confidence — simple)") and via `extraction.task_prompt_extraction_with_confidence_topk`. Advanced (agentic) integrated mode can opt into the same technique with the hidden `extraction.agentic.integrated_confidence_strategy: topk` knob. See the [reference config](config_library/unified/realkie-fcc-verified/config-1s-topk-with-ocr-image.yaml), the [demo notebook](notebooks/misc/e2e-example-with-1s-topk.ipynb), and the [extraction README](lib/idp_common_pkg/idp_common/extraction/README.md).

- **Self-healing confidence assessment for large lists (token-aware sizing + model-escalation ladder).** Advanced mode previously could silently leave rows with `confidence: null` when a small-cap confidence model (Nova Lite, 10K output) truncated a large batch — most acutely with `geometry.mode: llm_grounded`, whose per-cell bounding-box block roughly triples per-row output. Two additions make it complete correctly on the first pass: (1) the first-pass batch size is now **derived from the confidence model's output cap** (shrinking `list_batch_size` only when needed — e.g. Nova Lite + `llm_grounded` starts at ~6–9 rows instead of truncating at 25); and (2) when rows still come back unscored after shrinking + same-model retries, a bounded **escalation ladder re-assesses only the still-missing rows on a stronger confidence model** (bigger output cap). ON by default for advanced mode via `extraction.confidence.{escalation_enabled, escalation_model, max_escalation_rounds}` (per-class override `x-aws-idp-confidence-escalation-model`); the activity is recorded in `assessment_batch_split_stats` and the processing report. See the [assessment README](lib/idp_common_pkg/idp_common/assessment/README.md).

- **Structured processing issues surface what couldn't be auto-fixed.** A new `ProcessingIssue` concept (stage, severity, code, message, root cause, details) is produced by the backend, attached to each `Section` (rolled up to `Document.processing_issue_count`), round-tripped through `to_dict`/`from_dict`, and rendered in the processing report. Instead of a document silently completing with unscored rows, the section now carries e.g. `assessment_incomplete` (**error**, with the model/cap/rows root cause), `assessment_recovered_with_retries` (**info** — self-healed but token-inefficient), or `assessment_deadline_reached` (**warning**). A **completeness gate** (`audit_explainability`) verifies every extracted value has a valid, in-range confidence and (per geometry mode) a bounding box, feeding any structural gaps into the same issue list.

- **Processing issues are surfaced in the UI (status icon + hover details + report).** The structured `ProcessingIssue`s are persisted to DynamoDB (per-section `ProcessingIssues` + a top-level `ProcessingIssueCount` and sparse `HasProcessingIssues` GSI attribute), exposed over GraphQL (`Section.ProcessingIssues`, `Document/DocumentListItem.ProcessingIssueCount`, new `ProcessingIssue` type), and rendered in the web UI: a **Status** column on the Sections panel (worst-severity Cloudscape `StatusIndicator` with a hover popover listing each issue's message + root cause), a **Processing Issues** column on the document list, and a structured issue list at the top of the section **Processing Report** tab. So a document that self-healed with retries — or one where a row genuinely couldn't be scored — is now visible at a glance instead of only in the per-section `result.json`.

- **Assessment self-healing is bounded by the Lambda wall-clock budget and safe to retry.** The escalation ladder AND the adaptive batch-splitter read the Lambda's remaining time and **stop before starting work that wouldn't fit** (90s reserve), emitting an `assessment_deadline_reached` warning and keeping recovered rows rather than risking a hard task timeout. The wall-clock deadline is threaded through **every** entry point that runs the confidence ladder — the standalone Assessment step, the standalone Extraction Lambda, and (now) the Step Functions **per-shard and merge** handlers (`run_one_section_shard` / `merge_section_shards`) — so a small-cap confidence model that keeps truncating a large batch degrades gracefully instead of fanning one batch into dozens of ~60s calls that run a shard/merge Lambda into its 900s wall. The Step Functions `ExtractionStep`/`AssessmentStep`/`ShardExtractionStep` retry sets also include `States.Timeout` and `Lambda.Unknown`, so a genuine task timeout is retried (and resumes via the existing per-shard S3 persistence + `explainability_info` skip) instead of hard-failing the document.

- **Confidence assessment is text-only in OCR-geometry modes, right-sized, salvage-on-truncation, and resumable.** A cluster of fixes makes large-table confidence reliable and cheap on both the agentic in-shard path and the standalone Assessment step: (1) in `geometry.mode: ocr_only`/`off` the page images are **omitted from the confidence prompt** (geometry comes from OCR grounding, not the model) — dropping ~1.7K input tokens per image (a 5-page shard sheds ~8.7K) and removing the multimodal over-generation that pushed a small confidence model to its output-token ceiling; (2) the token-aware first-pass batch size is now derived from the row's **column count** (the real driver of confidence output) rather than value length, with a 50%-of-cap target so the first call fits with headroom instead of truncating; (3) a truncated response is **salvaged** — the valid prefix rows are recovered (via `repair_truncated_json`) and only the remainder is re-scored, instead of discarding the whole call; (4) the concurrent fan-out **inherits the batch size the cache-warm batch proved** so a truncation isn't independently re-discovered by every worker; and (5) per-shard extraction is **checkpointed to S3 before** the confidence pass, so a shard that runs out of wall-clock during assessment resumes by re-running assessment only — never repeating the expensive extraction agent loop.

- **Bounding-box OCR grounding is ~30× faster on large tables (indexed match).** Grounding matches each extracted value to its OCR line; on a big multi-page table (e.g. 1,440 rows × 6 columns) the old per-value linear scan over every page line re-normalized ~370 lines on every one of ~8,600 lookups and always ran the fuzzy/Levenshtein sweeps — ~60s+ per shard, enough to approach the Lambda's 900s wall silently. Grounding now builds a cached normalized-line list + an **exact-text index per page** and does an index-first pass (EXACT tier always wins), falling back to the full fuzzy ladder only when nothing matched verbatim; a tier-aware early-out skips span/fuzzy passes that can't beat what already matched. Result: ~64s → ~2s for a 1,440-row section, byte-identical output to the full scan, with a per-shard grounding duration now logged for traceability.

- **Quick Start: cold-start configuration authoring for new deployments.** A conversational Quick Start agent bootstraps a working configuration for first-time users with no prior setup — describe your document type in natural language, or attach example documents (PDF/image/ZIP) to run multi-document Discovery and infer document classes automatically. The agent authors an extraction schema, matches against a catalog of bundled sample configs, and writes the result as a new configuration version you can refine in chat. Whenever it names a bundled sample — an exact match or a related reference — it now **cites it as an inline link** (via a new `getSampleDocumentUrl` presigned-URL resolver, Admin/Author/Viewer) so you can open the real document from chat; for multi-file batch samples it links one representative file (the manifest records each batch's inner files). A single attached document is routed through single-document Discovery (so it appears on the **Single Document** tab) while multiple files still use multi-document Discovery. Surfaced via a floating Quick Start widget and a Welcome page for empty deployments; gated by the `EnableQuickStartWidget` stack parameter. Also available from the CLI (`idp-cli bootstrap`).

- **Claude Sonnet 5 is selectable everywhere** (`us`/`eu`/`global.anthropic.claude-sonnet-5`, plus the `:1m` extended-context variants) — registered in the model picklists, pricing, output-token limits (128K), prompt-cache support, and Bedrock TPM quota codes. Sonnet 5 rejects non-default `temperature`/`top_p`/`top_k`, so IDP strips those sampling params automatically. **Request Anthropic Claude Sonnet 5 access in Bedrock before selecting it.**

- **Reasoning effort is configurable for reasoning-capable Claude models (extraction defaults to `low`).** `reasoning_effort` now drives Claude's `output_config.effort` on the Bedrock Converse path for effort-capable models (Sonnet 5, Sonnet 4.6, Opus 4.5–4.8, Fable 5 — accepting `low`/`medium`/`high`/`xhigh`/`max`), in addition to OpenAI GPT-5.x. It is wired through both the standard and agentic (Strands) extraction paths, and the config UI shows the **Reasoning effort** selector only when the selected model supports it. A full effort sweep found higher effort adds output-token cost with negligible extraction-accuracy gain, so `low` keeps the Sonnet 5 default affordable — raise it per-config for genuinely reasoning-heavy documents.

- **Integrated confidence works on both the Advanced (agentic) and Simple (non-agentic) paths.** `confidence.mode: integrated` produces each value's confidence in the extraction inference itself — no separate assessment pass — then enriches with per-field thresholds + alerts, reconciles to full per-cell coverage, grounds geometry, and emits `explainability_info` (the standalone Assessment step auto-skips). See the [Extraction & Confidence guide](docs/extraction-and-confidence.md#confidence-mode-separate-vs-integrated-vs-off).

- **"View / Edit Model Limits" page in the web UI — per-model token limits are now editable at runtime.** The ordered, first-match-wins model limits list (`config_library/model_config_limits.yaml`) is now seeded into the DynamoDB Configuration Table at deploy (`DefaultModelConfigLimits`, mirroring pricing) and surfaced in a new Admin-editable page with Table / JSON / YAML views, row reordering (order is semantic — the first matching regex wins), add/delete, import/export, and restore-defaults. Edits are stored as a full-replacement `CustomModelConfigLimits` record and **take real effect**: `get_model_max_output_tokens()` now reads the merged limits from DynamoDB (60-second cache per warm Lambda container, so changes apply to running workers within about a minute), falling back to the on-disk `config_library/` YAML when no table is configured (offline/CLI use). The copy of `model_config_limits.yaml` previously bundled inside the `idp_common` wheel is removed — `config_library/model_config_limits.yaml` is the single source of truth, same as `pricing.yaml`. New GraphQL operations: `getModelConfigLimits` (Admin/Author/Viewer), `updateModelConfigLimits` and `restoreDefaultModelConfigLimits` (Admin only). The page includes a **"Test a model ID"** tool (model picklist sourced from the pricing keys, plus free-text entry for unlisted/future IDs) that shows which entry a given model resolves to using the same first-match logic as the runtime, and validates that each saved pattern is a compilable regex.
...
- **GovCloud Deployment guide restructured around the two `idp-cli` paths (`--govcloud` with the Web UI vs. `--headless`).** `docs/govcloud-deployment.md` now opens with a two-column decision table and gives a complete copy-paste `idp-cli deploy --from-code .` command for each variant: Web UI internet-facing, Web UI + WAF IP allow-list, Web UI VPC-only (`ApiGatewayVisibility=PRIVATE`), headless vanilla, headless + Jobs REST API, and headless + bastion. It also **corrects a stale claim** that `--headless` auto-sets the `EnableHeadless=true` stack parameter — it has not since v0.5.16; the flag only transforms the template, and the Jobs REST API examples now pass `EnableHeadless=true,DeployInVPC=true,<VPC params>` explicitly (matching the template's `HeadlessRequiresVPC` fail-fast rule). Duplicated architecture/services-removed content is replaced with links to [GovCloud Architecture](docs/govcloud-architecture.md) and [GovCloud Operations](docs/govcloud-operations.md), and related docs no longer describe headless as "required" for GovCloud. The **GovCloud Architecture** page was rewritten to match: it now documents both variants (a `--govcloud` diagram showing API Gateway serving the UI, and the headless diagram), replaces the stale AppSync/GraphQL "services removed" inventory (with hardcoded resource counts from v0.4.x) with the actual resource groups stripped by the current `HeadlessTemplateTransformer`, and corrects the pattern claim — the unified workflow's Pipeline mode is the GovCloud default and **BDA mode is available in `us-gov-west-1`** (the old page said "Pattern 2 only"). The Headless Deployment guide and GovCloud Operations log-group naming (`PATTERNSTACK`, not `PATTERN2STACK`) got the same AppSync/pattern cleanup.

- **Config editor (View/Edit Config) now shows per-step guidance and a consistent enable-toggle label.** Each processing-step section (OCR, Classification, Extraction, Summarization, Rule Validation, HITL) renders a short description under its heading explaining what the step does and when it is skipped — the config editor previously dropped these descriptions for both top-level and nested collapsible sections. Classification notes it skips LLM classification when only one class is defined or a document-name/page-content RegEx matches; Extraction notes it skips a class with no defined attributes; OCR notes that Textract features (LAYOUT/TABLES/FORMS) or the BDA backend add layout/tables/forms support. The deterministic table parser's OCR dependency is now called out in its heading (**"Table Parsing Tool (Requires OCR TABLES enabled)"**), and the enable toggle is labeled **"Enabled"** consistently across HITL, Summarization, and Rule Validation (HITL was previously "Enable HITL review"). Schema-metadata + UI-rendering only — no processing-logic change; existing stacks running a Custom config surface the new descriptions once the Default schema record is re-seeded on deploy.
- **Textract `TABLES` is enabled by default in OCR (`ocr.features: [TABLES, LAYOUT]`).** Tables are common and the accuracy gain is large: `TABLES` makes Textract emit structured Table/Cell blocks that linearize into clean Markdown pipe-tables for the agentic table parser, producing more complete extraction *and* more accurate confidence/geometry — in validation on a 24-page brokerage statement it extracted all 1,440 rows (every page) where `LAYOUT`-only silently dropped ~5 pages (~300 rows). **Cost trade-off:** `TABLES`+`LAYOUT` (~$0.065/page) is ~16× the Textract line-item cost of `LAYOUT`-only (~$0.004/page), but for documents *with* tables it is typically more cost-effective end-to-end (fewer LLM extraction retries, fewer confidence truncations/re-batches, less downstream correction). For a **table-free corpus** (forms, prose, single-value docs), remove the `TABLES` entry from `ocr.features` to fall back to cheaper `LAYOUT`-only OCR. See [Configuration → Textract features](docs/configuration.md).
- **`max_tokens` is now optional everywhere — leave it empty to use the selected model's maximum output limit.** Previously `max_tokens` was a required knob for classification, summarization, chat, evaluation, discovery, and rule-validation/discovery (OCR was hardcoded to 4096), while extraction and confidence had already dropped it. Now every service treats it as an **optional cap**: the field is `Optional[int]` (default `null`; an empty string in stored config also means "unset"), and when unset the Bedrock client resolves the model's max output tokens from `model_config_limits.yaml` (the DynamoDB-editable "View / Edit Model Limits" list). Set a positive value only to cap output *below* the model max. System-default configs ship these fields empty, so a fresh deploy uses each model's full output budget — avoiding silent truncation when switching to a larger-context model. In the config UI these advanced inference params (Temperature, Top K, Top P, Max tokens, Reasoning effort) are now grouped into a collapsed **"Model parameters"** section directly under the model selector, since they rarely need changing. Existing configs with numeric `max_tokens` values continue to work as explicit caps.
- **Evaluation reports now show the per-field comparison Method and Weight for nested list/object fields.** For arrays of objects, the whole list is paired with `HUNGARIAN`, but each item field (e.g. `LineItems[].Amount`) is scored with its own comparator — which was only shown for top-level attributes, not in the nested "Nested Field Comparison" table or the JSON Visual Editor's "Show Evaluation" overlay. Both now display each nested field's `Method` and `Weight`, consistent with the top-level attributes table (columns: Match | Score | Weight | Method | Reason). Stickler drops the per-field comparator/weight from its comparison output, so these are re-derived on read from the translated schema (`x-aws-stickler-comparator`/`-threshold`/`-weight`) — no re-run of evaluation is needed for the report, and the Visual Editor overlay now also decorates per-list-item leaf fields that previously showed no evaluation status at all. Evaluation results serialized before this change render with an empty method and a default weight of `1.00`. See the [evaluation guide](docs/evaluation.md).

- **Config format v0.6: confidence & geometry are outputs of extraction; HITL is its own section (auto-migrated on read).** The former top-level `assessment` block is retired as the home for confidence scoring — settings move under `extraction`, and existing v0.5 configs are migrated automatically by an idempotent transform at both config-import and deploy-time merge, so custom configs and presets keep working unchanged:
  - `extraction.assessment_integration` → **`extraction.confidence.mode`** (`off` | `separate` (default) | `integrated`).
  - `assessment.{model, prompts, image, decoding, ...}` → **`extraction.confidence.*`**; `assessment.inshard_list_batch_size` → **`extraction.confidence.list_batch_size`**.
  - `assessment.geometry_mode` → **`extraction.geometry.mode`**, enum values renamed: `ocr_only` (default) | `llm_grounded` | `llm` | `off`.
  - `assessment.hitl_enabled` / `assessment.default_confidence_threshold` → new top-level **`hitl.{enabled, confidence_threshold}`**.

  A slim `assessment` block survives only as the standalone Assessment-step / pipeline-hook carrier. The config UI is reorganized to match: a top-level **Extraction mode** selector (Simple / Advanced), **Confidence scoring mode** and **Geometry mode** at the top of Extraction, and a new **Human-in-the-Loop (HITL) Review** section. See the [Extraction & Confidence guide](docs/extraction-and-confidence.md).

- **Default extraction model is now Claude Sonnet 5; default confidence model is Amazon Nova Lite.** Sonnet 5 gives the best extraction quality; confidence stays on the far cheaper Nova Lite because a live A/B on the OCR benchmark measured **~$0.0011/doc on Nova Lite vs ~$0.145/doc on Sonnet 5 (~130×)** for the assessment pass, where verbose output tokens dominate cost. Presets that pin an explicit model are unaffected. **Request Anthropic Claude Sonnet 5 access in Bedrock before upgrading.**

- **Confidence prompts emit `confidence_reason` only for lower-confidence leaves (below 0.9), capped at ~12 words.** A high-confidence value now returns just `{"confidence": <score>}`. Because output tokens dominate assessment cost, this cuts confidence output substantially with no change to scores, thresholds, or alerts. Applies to both the separate-pass and integrated confidence prompts; to restore a reason on every field, widen the 0.9 threshold in the confidence `task_prompt`.

- **OCR-only geometry is the default** (`geometry.mode: ocr_only`) — field bounding boxes are derived by matching extracted values to real OCR lines in `pageData.json` instead of asking the model for coordinates. Cheaper (no bbox tokens) and more accurate (no hallucinated boxes); repeated values are disambiguated by row order, and matching is format-aware (dates, currency, and phone numbers still ground when canonicalized to a different surface form). LLM-box modes (`llm`, `llm_grounded`) remain available. See the [Extraction & Confidence guide](docs/extraction-and-confidence.md#geometry--bounding-boxes).

- **Advanced (agentic) extraction shards large documents reliably and runs on a Step Functions Distributed Map.** Sharding now engages by default on large docs (token and page-count bounds per shard), each shard persists idempotently to S3 so SFN retries re-run only failed shards, and confidence assessment runs **inside each shard** (scaling with extraction) — collated, reconciled to the exact row count, and emitted as the same `explainability_info` contract. See the [Extraction & Confidence guide](docs/extraction-and-confidence.md#large-document-guidance).

- **Bounding-box OCR grounding is now sharded (was a single full-section merge sweep).** Each shard grounds its own rows' geometry against only its own pages, right after its in-shard confidence pass — so grounding scales and parallelizes per-shard exactly like confidence assessment does. The merge step only grounds any residual rows (`skip_grounded`), collapsing what was previously an `O(rows × all-pages)` fuzzy-matching sweep that could approach the merge Lambda's 900s ceiling on very large tables (e.g. 1,400+ rows × 24 pages) down to near-instant. Scoping to each shard's pages also removes cross-page false matches. No config or output-format change.

- **Extraction & confidence always request the model's maximum output; `max_tokens` is no longer a config knob for them.** The Bedrock Converse default when `maxTokens` is omitted is a small truncating value, so the client now sets it explicitly from the per-model limits in `config_library/model_config_limits.yaml` (also bundled into `idp_common` so it resolves at Lambda runtime). The `extraction.max_tokens` and `extraction.confidence.max_tokens` fields are removed (leftover values are ignored — no migration needed); completeness matters more than an output cap for these passes. If a request still exceeds the model limit, the client parses the real cap from Bedrock's `ValidationException` and retries once. `classification`/`summarization` keep their `max_tokens` knob.

- **AWS AppSync fully removed — replaced by an API Gateway REST API + polling + Lambda response streaming.** The UI↔backend transport no longer uses AppSync (unavailable in GovCloud, not FedRAMP-compliant). UI queries/mutations go through a REST API with a Cognito authorizer (supporting `ApiGatewayVisibility=PRIVATE` and an optional WAF WebACL); status updates poll; the two chat flows stream via a direct Lambda Function URL. All `AWS::AppSync::*` resources are deleted and the Feature Platform is migrated. Parameters `AppSyncVisibility`/`UsePrivateAppSync` are renamed to **`ApiGatewayVisibility`/`UsePrivateApi`**. See the [AppSync → REST migration guide](docs/migration-appsync-to-rest.md).

### Removed

- **ALB Web UI hosting (`WebUIHosting=ALB`) removed — replaced by API Gateway hosting.** The internal-ALB-plus-S3-VPC-Interface-Endpoint hosting mode, the `nested/alb-hosting/` stack, and all `ALB*` parameters (`ALBVpcId`, `ALBSubnetIds`, `ALBCertificateArn`, `ALBScheme`, `ALBAllowedCIDRs`) have been deleted. `WebUIHosting` now accepts `CloudFront` (default) and `APIGateway` (see Added). **Migration:** switch `WebUIHosting=ALB` → `WebUIHosting=APIGateway`; for the equivalent private-network posture also set `ApiGatewayVisibility=PRIVATE` + `DeployInVPC=true` and provide `VpcId`/`PrivateSubnetIds`/`LambdaSubnetIds`/`LambdaSecurityGroupId`/`ApiGatewayVpcEndpointId` (an execute-api interface endpoint). No ACM certificate, ALB, or auto-provisioned S3 VPC endpoint is needed anymore. BYO S3 endpoint for browser presigned uploads is still supported via `S3VpcEndpointIdOverride`/`S3VpcEndpointDnsNameOverride` + `S3PresignedUrlViaVpcEndpoint=true`. The application URL changes from the ALB DNS name to the execute-api `/api/` URL (`ApplicationWebURL` output). Private-mode Lambda VPC wiring now keys off the standard `VpcId`/`DeployInVPC` parameters (previously reused `ALBVpcId`). See the [API Gateway Hosting guide](docs/apigateway-hosting.md).

- **Deprecated pre-Stickler evaluation engine deleted.** Now that evaluation runs entirely through the Stickler library (`evaluation/service.py`), the dead legacy modules have been removed: `evaluation/service_legacy.py` (the old custom `EvaluationService`, no live callers) and `evaluation/comparator.py` (the `Comparator` classes and `compare_exact`/`compare_fuzzy`/`compare_numeric`/`compare_hungarian`/`compare_semantic`/`compare_values`/`compare_date` helpers). The two functions still needed at runtime were relocated: `compare_llm` now lives in `evaluation/llm_comparator.py` (its only caller, the Stickler-registered `LLMComparator`), and `fuzz_score`/`strip_punctuation_space` moved to a new dependency-free `evaluation/text_matching.py` (used by OCR bounding-box grounding). The `idp_common.evaluation` package no longer re-exports the `compare_*` helpers, and the `evaluation_methods_demo.ipynb` notebook (which demoed the removed `compare_values` API) is retired. No YAML-config or runtime-behavior impact; affects only custom code that imported those internal symbols directly.

- **Granular assessment is retired and its service deleted.** The `GranularAssessmentService`, the `granular_service.py` module, and the `extraction.confidence.granular` config field no longer exist. Large-list assessment is now handled by **standalone large-list batching** (`extraction.confidence.list_batch_size`, default 25): the model scores each large list in sequential batches, then any dropped rows are re-scored by a bounded missing-row retry, so per-cell coverage reaches 100%. On a 120-row bank statement this was **~78% cheaper** than granular with equal accuracy and full per-cell confidence + geometry. **Migration is a no-op:** leftover `granular.*` keys still validate and are ignored. The standalone Assessment step's `postAssessment` pipeline-hook point is also removed. See [Granular Assessment Retirement](docs/migration-granular-retirement.md).

- **`idp_common` code-level breaking changes (affects custom scripts and notebooks, not YAML config).** The config auto-migration keeps *runtime* behavior working, but code touching the removed Python symbols breaks:
  - **`from idp_common.assessment import GranularAssessmentService`** (or `idp_common.assessment.granular_service`) now raises `ImportError`. The public `AssessmentService` / `create_assessment_service` entry points are unchanged and always run the standalone batched path; delete any granular-specific imports.
  - **`IDPConfig` no longer has a top-level `assessment` attribute** — read confidence/geometry from **`config.extraction.confidence.*`** / **`config.extraction.geometry.*`** and HITL from **`config.hitl.*`**. (A v0.5-shaped `{"assessment": {...}}` *dict* still migrates when passed to `IDPConfig(**cfg)`; the *typed* attribute is gone.)
  - `ExtractionService` / `AssessmentService` public method signatures are unchanged. New additive exports: the runtime-agnostic sharding primitives in `idp_common.extraction.runtime`, plus `assessment.batching` / `assessment.ocr_grounding`.
  - The `step4_assessment_granular.ipynb` notebooks are retired; `step4_assessment.ipynb` is now the single confidence/assessment notebook.

### Fixed

- **Agent jobs (Error Analyzer / Agent Companion Chat) no longer get stuck in `PENDING`.** After the AppSync→REST migration, the `AgentProcessor` Lambda's only status-persistence path was a GraphQL mutation to AppSync — which no longer exists, so `APPSYNC_API_URL` was empty and every `COMPLETED`/`FAILED` write threw `MissingSchema: Invalid URL ''` and was dropped. The agent finished server-side and its transcript/result were produced (the message transcript is written separately), but the job's `status` stayed at the initial `PENDING` forever, so the polling UI never showed the result — and the Troubleshoot modal's "Report this issue on GitHub" button (gated on `COMPLETED`) never appeared. The processor now writes `status`/`result`/`error`/`completedAt` **directly to the Agent DynamoDB table** (the source of truth the `getAgentJobStatus` poller reads); the dead AppSync mutation path (and its now-unused `requests` / `aws-requests-auth` dependency) was removed. No IAM change (the function already has `DynamoDBCrudPolicy` on the table).

- **Reviewers can save section edits again in the Visual Editor (HITL review).** Clicking **Save All Changes** after editing fields as a Reviewer failed with `Not Authorized to access uploadDocument on type Mutation` (403, errorType `Unauthorized`). The editor's save path requested a presigned upload URL via the `uploadDocument` mutation, which RBAC restricts to Admin/Author, so Reviewers were denied. The reviewer save now routes through the reviewer-permitted `completeSectionReview` mutation (Admin/Reviewer), which writes the edited JSON to the section's output URI server-side and records the section review — no presigned URL needed; it is registered in both AppSync and the REST HTTP API dispatcher. Reviewer status is derived from the Cognito token via `useUserRole` (`canReview && !canWrite`) rather than the `isReviewerOnly` prop, which some `SectionsPanel` `FileViewer` entry points hardcode to `false`. Admin/Author keep the presigned `uploadDocument` path; baseline edits remain Admin/Author-only. Hardening: the `completeSectionReview` resolver now **raises instead of silently succeeding** when edited data is supplied but the section's output URI cannot be resolved (previously the section could be marked reviewed with the edits dropped), and the editor no longer discards a reviewer's baseline edits on save — it keeps them and reports that baseline edits require an Author/Admin role.

- **Config-version scope was silently unenforced for configuration reads (fail-open).** The `ConfigurationResolverFunction` in `nested/api-resolvers/template.yaml` sets the `USERS_TABLE_NAME` env var and looks up the caller's `allowedConfigVersions` on the UsersTable `EmailIndex` to enforce config-version scope, but its IAM policy granted DynamoDB access only to the ConfigurationTable — not the UsersTable. Every scope lookup therefore failed with `AccessDeniedException`, which the resolver catches and treats as "no restriction", so a config-version-scoped non-admin user was silently granted **unrestricted** access to every configuration version (`getConfigVersion`, `getConfigVersions`, `getPricing`, `getModelConfigLimits`). The resolver's role now grants `dynamodb:Query`/`GetItem` on the UsersTable and its `/index/*` (the same grant its peer scope-enforcing resolvers — reprocess, sync-BDA, list-documents — already had). Found by the new API RBAC test harness (`make api-test`), whose live scoped-user suite asserts an out-of-scope version is denied. No other resolver was affected.

- **CORS preflight 500 broke every web-UI API call under `BinaryMediaTypes: */*`.** The api-resolvers REST API sets `BinaryMediaTypes: */*` so the S3-proxy Web UI routes can return raw asset bytes, but `*/*` applies API-wide: browsers send `Accept: */*` on the unauthenticated CORS `OPTIONS` preflight, so API Gateway tried to base64-encode the MOCK preflight response as binary and returned a platform-level 500. A non-2xx preflight makes the browser abort the follow-up POST with `Failed to fetch`, so the UI couldn't load configuration, documents, or any other data. The `OPTIONS` MOCK integration (and its 200 response) now force `ContentHandling: CONVERT_TO_TEXT`, keeping the preflight a clean 200; the binary Web UI GET routes are unaffected (they keep `CONVERT_TO_BINARY`).

- **Agent Companion Chat and Quick Start chat no longer time out without a reply on non-streaming (GovCloud / API Gateway) deployments.** On deployments without the Lambda Function URL streaming endpoint, the UI sends the chat message over the REST API and polls the chat-messages table for the persisted final answer. But the agent-chat resolver invoked the chat processor **without the caller's identity** (`callerSub`), and the processor silently skips writing the assistant reply to the table when the identity is empty — so the agent generated a complete answer that was never persisted, and the UI polled for 5 minutes before showing "Timed out waiting for the assistant response". (Chat-with-Document was unaffected; its resolver already passed the identity through.) The resolver now threads `callerSub` (username-first, matching the read-side session-ownership check) to the processor, plus a `persistUserMessage=False` flag so the user message the resolver already stored isn't double-written; the processor honors the flag, keeps the session `messageCount` accurate, and logs a clear warning instead of a silent no-op when identity is missing. The agent chat UI also gains a visible **waiting indicator** — an assistant-style bubble with a spinner, "Working on your request…", and the animated gen-AI loading bar — shown in both Agent Companion Chat and Quick Start while the reply is pending (previously only a thin loading bar that was easy to miss).

- **Documents with `#` in their filename no longer fail with `NoSuchKey` at every processing stage.** `urlparse` treats `#` as a URL fragment delimiter (RFC 3986) and silently truncated S3 keys at that character in `Document.decompress()`, `S3Util.s3_url_to_bucket_key()`, and the reporting data writer. All three now use the existing `parse_s3_uri()` utility, which splits the URI literally and preserves any key character.

- **A config that enables agentic/advanced extraction without an explicit `extraction.mode` is no longer silently reverted to simple single-pass.** In v0.6 `extraction.mode` is authoritative and `extraction.agentic.enabled` is derived from it (`ExtractionConfig.reconcile_mode_and_agentic`). A pre-0.6 / hand-written delta that set `extraction.agentic.enabled: true` but omitted `extraction.mode` was a footgun: after the delta deep-merges onto the v0.6 defaults (which carry `mode: simple`), reconcile flipped `agentic.enabled` back to `false`, so an author who asked for agentic extraction silently got simple single-pass — which then times out on large list documents (the root cause of the CI Step-8 Nuveen 532-row/17-page timeout, previously misattributed to a table-parser regression). The v0.5→v0.6 migration now **pins `mode` from `agentic.enabled`** on the delta *before* the merge, so the author's intent wins; it never overrides an explicitly-set `mode` and is idempotent. The CI Step-8 config (`scripts/sdlc/config/nuveen.yaml`) is also converted to native v0.6 format, and Step 8 is re-enabled.

- **Agentic table parser now merges multi-page tables whose continuation pages lost the header label.** The deterministic table parser (`parse_table`) recovers all rows of a table that spans many pages, but `_merge_adjacent_tables()` only merged fragments whose column headers were **exactly** equal. When OCR keeps the header on page 1 (e.g. `Fund Name`) but the header cell comes back **blank on continuation pages** (parsed as an `_unnamed_N` placeholder), the fragments stayed split into multiple tables — which (a) emitted a "multiple tables ... verify completeness" warning that pushed the extraction agent into a slow row-by-row fallback, and (b) mis-keyed continuation rows under `_unnamed_0` so `map_table_to_schema` dropped the named column's value on most rows. Header comparison is now **placeholder-tolerant** (blank / `_unnamed_*` cells match any column when every other column lines up) and the retained header is **upgraded** to keep every real column name, so a large multi-page table (validated on a 17-page, 532-row financial statement) collapses back into a single fully-named table and the fast `parse_table` → `map_table_to_schema` → `finalize_table_extraction` path stays on track. This fixes the CI large-table agentic-extraction timeout and benefits any multi-page continuation table.

- **`idp-cli test-result` / `test-compare` / abort work again against v0.6 stacks (AppSync → API resolver rename).** The Test Studio SDK looked up its resolver Lambdas in a nested stack matching `appsync`, but that stack was renamed `APIRESOLVERSTACK` in the AppSync removal, so every Test Studio CLI/SDK call failed with `Nested stack matching 'appsync' not found`. The lookup now targets `apiresolver` and falls back to `appsync` for stacks deployed from pre-rename templates. Also fixed abort's missing-resolver handler to catch the exception actually raised, so the "stack version does not support test run abort" message surfaces instead of a raw `ValueError`.

- **"View Data" / "Download" on large section results no longer fails with "Failed to load content".** The section-data viewer and its Download button fetched each section's `result.json` through the `getFileContents` resolver, which returns the file bytes inline in the Lambda response. For a large section (e.g. a 131-page single-section document whose `result.json` was ~5.7 MB), JSON string-escaping pushed the serialized response past **Lambda's hard 6 MB synchronous payload limit**, so the invocation threw (`Response payload size exceeded maximum allowed payload size`) and the UI showed a generic "Failed to load content. Please try again." A new **`getFilePresignedUrl`** query returns a short-lived (5-minute) presigned GET URL (served by the existing `GetFileContentsResolverFunction`, reusing its bucket allow-list + IAM; aliased in the HTTP API dispatcher so the field-function-map SSM parameter stays under its 8 KB Advanced-tier ceiling) so the browser fetches the bytes **directly from S3** — no size limit. The presigned URL is signed with **Signature Version 4** because the IDP buckets are SSE-KMS-encrypted and S3 rejects SigV2 for KMS-encrypted objects. The Visual Editor (`JSONViewer`) and the Sections panel Download now use this path; `getFileContents` remains for the smaller markdown/page-text viewers.

- **"Use as Evaluation Baseline" now actually updates the document's Evaluation status.** The copy-to-baseline resolver Lambda had the `TRACKING_TABLE` env var and code to write `EvaluationStatus` (`BASELINE_COPYING` → `BASELINE_AVAILABLE`/`BASELINE_ERROR`), but its IAM role was **missing DynamoDB write permission** — so the S3 copy succeeded while every status write failed with `AccessDeniedException` (caught and swallowed), leaving the UI stuck on `NO_BASELINE`/`COMPLETED` even after refresh. Added a `DynamoDBCrudPolicy` on the tracking table to the `CopyToBaselineResolverFunction`; the UI's 5-second document-list poll now picks up the new status automatically (no manual refresh). Also fixed the document detail view, which rendered the baseline status indicators component without passing it the document's `evaluationStatus`, so the "Baseline available / copying / failed" badges never appeared.

- **CLI/SDK: promote a processed document to the evaluation baseline programmatically.** New `idp-cli use-as-baseline --stack-name <s> --document-id <key>` command and `client.evaluation.use_as_baseline(document_id=...)` SDK method — the scriptable equivalent of the UI button. Runs synchronously (copies the document's output prefix into the evaluation baseline bucket and sets `EvaluationStatus=BASELINE_AVAILABLE`), returning the file count so batch/automation flows can verify success.

- **Evaluation no longer fails a whole document when a `required` field is correctly null.** A config marking a field `required` that a document genuinely lacked raised `Field required [type=missing]` after null-stripping, producing an `__EVALUATION_FAILURE__` that zero-scored **every** attribute in the section (deflating accuracy, inflating failure counts). Evaluation now clears `required` arrays (matching the auto-generated-schema path), so a missing/null field scores as a normal miss on that one attribute.

- **Confidence assessment no longer silently loses scores + bounding boxes when the model truncates its output.** A large per-row assessment batch (most visibly with `geometry.mode: llm`) could exceed a small-cap confidence model's max output tokens (e.g. Nova Lite's 10,000), yielding unparseable JSON that the service silently replaced with a default `0.5` and null-confidence placeholders. The core now **detects truncation** (`stopReason == "max_tokens"`, plus the OpenAI-Responses `incomplete` status; exposed as `AssessmentCoreResult.truncated`) and the large-list batcher **recursively halves any truncated slice and re-assesses it until it fits**, across every confidence path. Split activity is surfaced via `metadata.assessment_batch_split_stats` and an `⚠ Assessment Batch Splitting` block in the agentic report.

- **Reasoning models (Claude Sonnet 5, Sonnet/Opus 4.6+, any model with extended thinking) returned empty extractions/classifications.** These models emit `reasoningContent` blocks *before* the answer `text` block, but the response parsers read only `content[0].get("text")` — the reasoning block — yielding an empty string and silent 0% accuracy whenever the model chose to think. The parsers now concatenate all `text` blocks and skip `reasoningContent`. Affected extraction, classification, assessment, and summarization.
