...
- **Quick Start: cold-start configuration authoring for new deployments.** A conversational Quick Start agent bootstraps a working configuration for first-time users with no prior setup — describe your document type in natural language, or attach example documents (PDF/image/ZIP) to run multi-document Discovery and infer document classes automatically. The agent authors an extraction schema, matches against a catalog of bundled sample configs, and writes the result as a new configuration version you can refine in chat. When it discusses a document type that matches a bundled sample, it now **cites that sample as an inline link** (via a new `getSampleDocumentUrl` presigned-URL resolver, Admin/Author/Viewer) so you can open the real reference document from chat. A single attached document is routed through single-document Discovery (so it appears on the **Single Document** tab) while multiple files still use multi-document Discovery. Surfaced via a floating Quick Start widget and a Welcome page for empty deployments; gated by the `EnableQuickStartWidget` stack parameter. Also available from the CLI (`idp-cli bootstrap`).
