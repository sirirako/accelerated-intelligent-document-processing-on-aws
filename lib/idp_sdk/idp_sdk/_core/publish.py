        ...
        try:
            ...
            if components_needing_rebuild:
                ...
                replacements = {
                    ...
                    # Both the build hash (triggers DockerBuildRun + the
                    # AgentCore Runtime UpdateAgentRuntime) and the source-zip
                    # key are driven by the same content hash over every image
                    # input (Dockerfile, requirements, handler, generator), so a
                    # change to any of them rebuilds and redeploys the image.
