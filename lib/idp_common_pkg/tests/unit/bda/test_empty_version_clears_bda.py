...
class TestEmptyVersionClearsBda:
    ...
    def test_empty_version_clears_bda_blueprints(self, mock_blueprint_creator, service, existing_blueprints):
        ...
        assert mock_creator_instance.delete_blueprint.call_count == len(existing_blueprints)
        
        # Verify delete_blueprint was called with correct ARNs
        expected_calls = [
            (existing_blueprints[0]["blueprintArn"], existing_blueprints[0]["blueprintVersion"]),
            (existing_blueprints[1]["blueprintArn"], existing_blueprints[1]["blueprintVersion"])
        ]
        
        actual_calls = [call.args for call in mock_creator_instance.delete_blueprint.call_args_list]
        assert set(actual_calls) == set(expected_calls)

    @patch('idp_common.bda.bda_blueprint_service.BDABlueprintCreator')
    def test_empty_version_no_existing_blueprints(self, mock_blueprint_creator, service):
    ...
    @patch('idp_common.bda.bda_blueprint_service.BDABlueprintCreator')
    def test_version_with_classes_preserves_matching_blueprints(self, mock_blueprint_creator, service, existing_blueprints):
        ...
        mock_config = Mock()
        mock_config.classes = [
            {"$id": "Invoice", "x-aws-idp-document-type": "Invoice"}
        ]
        service.config_manager.get_configuration.return_value = mock_config
        ...
        service._process_classes_parallel = Mock(return_value=(
            [{"status": "success", "class": "Invoice"}],  # status
            ["arn:aws:bedrock:us-east-1:123456789012:blueprint/test-stack-Invoice-abc123"],  # updated
            False  # modified
        ))
