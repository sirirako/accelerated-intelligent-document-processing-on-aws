...
Builds all Lambda functions, Lambda layers, SAM templates, and the web UI,
then uploads everything to an S3 artifact bucket. The resulting template URL
can be used directly with `idp-cli deploy --template-url` or
...
**Optional Parameters:**
- `--source-dir`: Path to IDP project root directory (default: `.`)
- `--bucket-basename`: S3 bucket basename — region is appended automatically as `<basename>-<region>`. Auto-generated as `idp-accelerator-artifacts-<account-id>` if not provided.
- `--prefix`: S3 key prefix for artifacts (default: `idp-cli`)
- `--headless`: Also generate a headless (no-UI) template variant for GovCloud or API-only deployments
- `--public`: Make S3 artifacts publicly readable
- `--max-workers`: Maximum concurrent build workers (default: auto-detect)
- `--clean-build`: Delete all checksum files to force full rebuild
...
- `--artifacts-bucket-kms-key-arn ARN`: Apply SSE-KMS encryption to the artifact bucket using the specified customer-managed KMS key (CMK). When set, all uploaded objects (templates, Lambda zips, layers) are encrypted with this key. **Important:** Use a standalone KMS key that is NOT managed inside a CloudFormation stack — if the key is deleted or disabled, ALL S3 objects become inaccessible.
- `--artifacts-bucket-tags Key=Value,...`: Apply cost-allocation and governance tags to the artifact bucket. Accepts a comma-separated list of `Key=Value` pairs.
...
idp-cli publish --source-dir . --bucket-basename my-artifacts --prefix idp --region us-east-1

# Build with headless (GovCloud) template variant
idp-cli publish --source-dir . --region us-east-1 --headless

# Enterprise: KMS encryption on the artifact bucket
idp-cli publish --source-dir . --region us-east-1 \
    --bucket-basename my-artifacts \
    --artifacts-bucket-kms-key-arn arn:aws:kms:us-east-1:123456789012:key/mrk-abc123
...
# Force full rebuild with verbose output
idp-cli publish --source-dir . --region us-east-1 --clean-build --verbose
```

**Two-Step Workflow (publish then deploy):**

```bash
# Step 1: Build and publish artifacts
idp-cli publish --source-dir . --region us-east-1 --bucket-basename my-artifacts

# Step 2: Deploy using the published template URL
idp-cli deploy \
    --stack-name my-idp \
    --admin-email user@example.com \
    --template-url https://s3.us-east-1.amazonaws.com/my-artifacts-us-east-1/idp-cli/idp-main.yaml \
    --wait
```
