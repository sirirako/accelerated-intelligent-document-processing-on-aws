...
- **Per-attribute scoring**: Individual confidence scores and reasons for each field, recursively for nested groups and list items.
...
- **`integrated` on the simple path** is a true single inference: one Bedrock call returns values **and** confidence together as `{"extraction": {...}, "confidence": {...}}`. The service splits that envelope (values → `inference_result`, confidence → `explainability_info` after threshold-enrichment + OCR grounding), so the standalone Assessment step auto-skips (the single-inference cost win). If the model returns a flat response with no confidence, the path safely falls back to the standalone step. Best for **smaller documents** where the whole doc fits one inference; for large docs prefer agentic or `separate`. Any list rows the single inference leaves unscored are retried (missing rows only, bounded) so large-list coverage still reaches 100%.
